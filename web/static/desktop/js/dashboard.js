...
function installIndustrialDeckLayout() {
  ...
  if (motorControls && !document.getElementById("pointingModelEditorOpen")) {
    const editPointingModel = document.createElement("button");
    editPointingModel.className = "pointing-model-editor-open";
    editPointingModel.id = "pointingModelEditorOpen";
    editPointingModel.type = "button";
    editPointingModel.setAttribute("aria-haspopup", "dialog");
    editPointingModel.setAttribute("aria-controls", "pointingModelEditor");
    editPointingModel.setAttribute("aria-expanded", "false");
    editPointingModel.innerHTML = `
      <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20h4l11-11-4-4L4 16v4zM13.5 6.5l4 4M4 12V4h8M12 20h8v-8"></path></svg>
      <span>Edit Pointing Model</span>
      <b id="pointingModelEditorCount">0</b>`;
    const beacon = document.getElementById("industrialBeaconStatus");
    motorControls.insertBefore(editPointingModel, beacon || motorControls.firstChild);
  }
  ...
  const pointingCameraWindow = document.getElementById("pointingCameraWindow");
  if (monitorShell && pointingCameraWindow && !pointingCameraWindow.classList.contains("is-docked")) {
    pointingCameraWindow.classList.add("is-docked");
    pointingCameraWindow.removeAttribute("data-floating-window");
    pointingCameraWindow.removeAttribute("role");
    pointingCameraWindow.removeAttribute("aria-modal");
    pointingCameraWindow.style.removeProperty("left");
    pointingCameraWindow.style.removeProperty("right");
    pointingCameraWindow.style.removeProperty("top");
    pointingCameraWindow.style.removeProperty("bottom");
    pointingCameraWindow.style.removeProperty("width");
    pointingCameraWindow.style.removeProperty("height");
    pointingCameraWindow.style.removeProperty("transform");
    pointingCameraWindow.querySelector("[data-floating-window-handle]")?.removeAttribute("data-floating-window-handle");
    monitorShell.append(pointingCameraWindow);
  }
}

installIndustrialDeckLayout();

function installCameraStreamLoaders() {
  [["thermal", "Thermal"], ["visible", "Visible"]].forEach(([camera, label]) => {
    const viewport = document.querySelector(`.${camera}-feed .camera-viewport`);
    if (!viewport || viewport.querySelector(`[data-camera-loader="${camera}"]`)) return;
    const loader = document.createElement("div");
    loader.className = "camera-stream-loader";
    loader.dataset.cameraLoader = camera;
    loader.setAttribute("role", "status");
    loader.setAttribute("aria-live", "polite");
    loader.innerHTML = `<i aria-hidden="true"></i><span><strong>Connecting ${label} Camera</strong><small>Establishing JPEG stream…</small></span>`;
    viewport.append(loader);
  });
}

installCameraStreamLoaders();
...
const fields = {
  ...
  cameraCalibrationOpen: document.getElementById("cameraCalibrationOpen"),
  cameraCalibrationWindow: document.getElementById("cameraCalibrationWindow"),
  cameraCalibrationClose: document.getElementById("cameraCalibrationClose"),
  cameraCalibrationPreviews: document.getElementById("cameraCalibrationPreviews"),
  cameraCalibrationThermalCanvas: document.getElementById("cameraCalibrationThermalCanvas"),
  cameraCalibrationVisibleCanvas: document.getElementById("cameraCalibrationVisibleCanvas"),
  cameraCalibrationVisiblePosition: document.getElementById("cameraCalibrationVisiblePosition"),
  cameraCalibrationVisibleResolution: document.getElementById("cameraCalibrationVisibleResolution"),
  cameraCalibrationOffsetX: document.getElementById("cameraCalibrationOffsetX"),
  cameraCalibrationOffsetY: document.getElementById("cameraCalibrationOffsetY"),
  cameraCalibrationStep: document.getElementById("cameraCalibrationStep"),
  cameraCalibrationCropWidth: document.getElementById("cameraCalibrationCropWidth"),
  cameraCalibrationCropHeight: document.getElementById("cameraCalibrationCropHeight"),
  cameraCalibrationMessage: document.getElementById("cameraCalibrationMessage"),
  cameraCalibrationReset: document.getElementById("cameraCalibrationReset"),
  cameraCalibrationSave: document.getElementById("cameraCalibrationSave"),
  ...
  pointingCameraOpen: document.getElementById("pointingCameraOpen"),
  pointingCameraWindow: document.getElementById("pointingCameraWindow"),
  pointingCameraClose: document.getElementById("pointingCameraClose"),
  pointingCameraPopout: document.getElementById("pointingCameraPopout"),
  pointingCameraSource: document.getElementById("pointingCameraSource"),
  pointingCameraThermalImage: document.getElementById("pointingCameraThermalImage"),
  pointingCameraVisibleImage: document.getElementById("pointingCameraVisibleImage"),
  pointingCameraThermalZoomPct: document.getElementById("pointingCameraThermalZoomPct"),
  pointingCameraVisibleZoomPct: document.getElementById("pointingCameraVisibleZoomPct"),
  pointingCameraDirectionMode: document.getElementById("pointingCameraDirectionMode"),
  pointingCameraDirectionValue: document.getElementById("pointingCameraDirectionValue"),
  pointingCameraDirectionValueLabel: document.getElementById("pointingCameraDirectionValueLabel"),
  pointingCameraDirectionUnit: document.getElementById("pointingCameraDirectionUnit"),
  pointingCameraDirectionPad: document.getElementById("pointingCameraDirectionPad"),
  pointingCameraDirectionMessage: document.getElementById("pointingCameraDirectionMessage"),
  ...
  cameraMetadataPanel: document.getElementById("cameraMetadataPanel"),
  cameraMetadataTitle: document.getElementById("cameraMetadataTitle"),
  cameraMetadataSource: document.getElementById("cameraMetadataSource"),
  cameraMetadataBody: document.getElementById("cameraMetadataBody"),
  cameraMetadataClose: document.getElementById("cameraMetadataClose"),
  ...
  directionKicker: document.getElementById("directionControlKicker"),
  directionMode: document.getElementById("directionControlMode"),
  directionSpeed: document.getElementById("directionControlSpeed"),
  directionValueLabel: document.getElementById("directionControlValueLabel"),
  directionUnit: document.getElementById("directionControlUnit"),
  directionPad: document.getElementById("directionControlPad"),
  directionMessage: document.getElementById("directionControlMessage"),
  directionSafety: document.getElementById("directionControlSafety"),
  ...
  settingsActionMessage: document.getElementById("settingsActionMessage"),
  ...
  pointingModelEditorOpen: document.getElementById("pointingModelEditorOpen"),
  pointingModelEditorCount: document.getElementById("pointingModelEditorCount"),
  pointingModelEditor: document.getElementById("pointingModelEditor"),
  pointingModelEditorClose: document.getElementById("pointingModelEditorClose"),
  pointingModelEditorStatus: document.getElementById("pointingModelEditorStatus"),
  pointingModelEditorBody: document.getElementById("pointingModelEditorBody"),
  pointingModelEditorEmpty: document.getElementById("pointingModelEditorEmpty"),
  pointingModelEditorModelState: document.getElementById("pointingModelEditorModelState"),
  ...
  pointingLiveAzimuthPosition: document.getElementById("pointingLiveAzimuthPosition"),
  pointingLiveAltitudePosition: document.getElementById("pointingLiveAltitudePosition"),
  pointingLiveAzimuthVelocity: document.getElementById("pointingLiveAzimuthVelocity"),
  pointingLiveAltitudeVelocity: document.getElementById("pointingLiveAltitudeVelocity"),
  ...
  pointingTargetSlantRange: document.getElementById("pointingTargetSlantRange"),
  ...
  pointingTargetCurvature: document.getElementById("pointingTargetCurvature"),
  pointingTargetAlignment: document.getElementById("pointingTargetAlignment"),
  ...
  pointingModelComparisonPanel: document.getElementById("pointingModelComparisonPanel"),
  pointingModelComparisonClose: document.getElementById("pointingModelComparisonClose"),
  pointingModelComparisonGrid: document.getElementById("pointingModelComparisonGrid"),
  pointingModelLatitude: document.getElementById("pointingModelLatitude"),
  pointingModelLongitude: document.getElementById("pointingModelLongitude"),
  pointingModelDemAzimuth: document.getElementById("pointingModelDemAzimuth"),
  pointingModelDemAltitude: document.getElementById("pointingModelDemAltitude"),
  pointingModelTelemetryAzimuth: document.getElementById("pointingModelTelemetryAzimuth"),
  pointingModelTelemetryAltitude: document.getElementById("pointingModelTelemetryAltitude"),
  pointingModelAzimuthDifference: document.getElementById("pointingModelAzimuthDifference"),
  pointingModelAltitudeDifference: document.getElementById("pointingModelAltitudeDifference"),
  pointingModelLinkedZoomPct: document.getElementById("pointingModelLinkedZoomPct"),
  pointingModelZoomStep: document.getElementById("pointingModelZoomStep"),
  pointingModelZoomOut: document.getElementById("pointingModelZoomOut"),
  pointingModelZoomIn: document.getElementById("pointingModelZoomIn"),
  pointingModelNext: document.getElementById("pointingModelNext"),
  pointingModelZoomMessage: document.getElementById("pointingModelZoomMessage"),
  pointingModelComparisonMessage: document.getElementById("pointingModelComparisonMessage"),
  pointingAlignmentPanel: document.getElementById("pointingAlignmentPanel"),
  pointingAlignmentClose: document.getElementById("pointingAlignmentClose"),
  pointingAlignmentMinimize: document.getElementById("pointingAlignmentMinimize"),
  pointingAlignmentMinimized: document.getElementById("pointingAlignmentMinimized"),
  pointingAlignmentRestore: document.getElementById("pointingAlignmentRestore"),
  pointingAlignmentMinimizedStatus: document.getElementById("pointingAlignmentMinimizedStatus"),
  pointingAlignmentGrid: document.getElementById("pointingAlignmentGrid"),
  pointingAlignmentRawAzimuth: document.getElementById("pointingAlignmentRawAzimuth"),
  pointingAlignmentRawAltitude: document.getElementById("pointingAlignmentRawAltitude"),
  pointingAlignmentThermalImage: document.getElementById("pointingAlignmentThermalImage"),
  pointingAlignmentVisibleImage: document.getElementById("pointingAlignmentVisibleImage"),
  pointingAlignmentDemAzimuth: document.getElementById("pointingAlignmentDemAzimuth"),
  pointingAlignmentDemAltitude: document.getElementById("pointingAlignmentDemAltitude"),
  pointingAlignmentOffsetAzimuth: document.getElementById("pointingAlignmentOffsetAzimuth"),
  pointingAlignmentOffsetAltitude: document.getElementById("pointingAlignmentOffsetAltitude"),
  pointingAlignmentAdjustedAzimuth: document.getElementById("pointingAlignmentAdjustedAzimuth"),
  pointingAlignmentAdjustedAltitude: document.getElementById("pointingAlignmentAdjustedAltitude"),
  pointingAlignmentTelemetryAzimuth: document.getElementById("pointingAlignmentTelemetryAzimuth"),
  pointingAlignmentTelemetryAltitude: document.getElementById("pointingAlignmentTelemetryAltitude"),
  pointingAlignmentResidualAzimuth: document.getElementById("pointingAlignmentResidualAzimuth"),
  pointingAlignmentResidualAltitude: document.getElementById("pointingAlignmentResidualAltitude"),
  pointingAlignmentLinkedZoomPct: document.getElementById("pointingAlignmentLinkedZoomPct"),
  pointingAlignmentZoomStep: document.getElementById("pointingAlignmentZoomStep"),
  pointingAlignmentZoomOut: document.getElementById("pointingAlignmentZoomOut"),
  pointingAlignmentZoomIn: document.getElementById("pointingAlignmentZoomIn"),
  pointingAlignmentZoomMessage: document.getElementById("pointingAlignmentZoomMessage"),
  pointingAlignmentDirectionMode: document.getElementById("pointingAlignmentDirectionMode"),
  pointingAlignmentDirectionValueLabel: document.getElementById("pointingAlignmentDirectionValueLabel"),
  pointingAlignmentDirectionUnit: document.getElementById("pointingAlignmentDirectionUnit"),
  pointingAlignmentDirectionMessage: document.getElementById("pointingAlignmentDirectionMessage"),
  pointingAlignmentOffsetStep: document.getElementById("pointingAlignmentOffsetStep"),
  pointingAlignmentDirectionPad: document.getElementById("pointingAlignmentDirectionPad"),
  pointingAlignmentBack: document.getElementById("pointingAlignmentBack"),
  pointingAlignmentSave: document.getElementById("pointingAlignmentSave"),
  pointingAlignmentMessage: document.getElementById("pointingAlignmentMessage"),
  pointingAlignmentPointCount: document.getElementById("pointingAlignmentPointCount"),
  pointingAlignmentModelError: document.getElementById("pointingAlignmentModelError"),
...
// Visibility and grid are optional analysis overlays. Start each Pointing
// Model session with an uncluttered terrain view, including pages rendered
// from an older cached template that still has checked attributes.
if (fields.pointingLayerVisibility) fields.pointingLayerVisibility.checked = false;
if (fields.pointingLayerGrid) fields.pointingLayerGrid.checked = false;
...
async function fetchWithTimeout(url, options = {}, timeoutMs = SETTINGS_REQUEST_TIMEOUT_MS) {
...
let activeDirectionMode = "jog";
const directionModeValues = { jog: "5.0", offset: "1.000" };
let activePointingCameraDirectionMode = "jog";
const pointingCameraDirectionValues = { jog: "5.0", offset: "1.000" };
...
let appSettings = {
  ...
  pointing_alignment: { latest: null, points: [], model: { active: false, point_count: 0 } },
  motion_limits: {},
...
let pointingModelComparisonSample = null;
let pointingModelZoomInFlight = false;
let pointingModelLinkedZoomPct = null;
let pointingAlignmentSample = null;
let pointingAlignmentOffset = { azimuth: 0, altitude: 0 };
const pointingAlignmentDirectionValues = { jog: "1.0", offset: "0.100" };
let activePointingAlignmentDirectionMode = "offset";
let selectedPointingModelPointId = "";
let pointingAlignmentFrameSession = 0;
const pointingAlignmentFrameTimers = new Map();
let pointingSampleController = null;
let pointingSampleRequestSequence = 0;
...
function renderStatus(data) {
  ...
  if (fields.pointingModelComparisonPanel && !fields.pointingModelComparisonPanel.hidden) {
    renderPointingModelComparison();
  }
  if (fields.pointingAlignmentPanel && !fields.pointingAlignmentPanel.hidden) {
    renderPointingAlignment();
  }
  if (fields.navDot) {
...
function renderIndustrialSummary(data) {
  ...
  const renderPointingTelemetry = (field, value, unit) => {
    if (!field) return;
    const numericValue = connected ? toNumber(value) : null;
    field.textContent = numericValue === null ? "--" : `${formatSignedNumber(numericValue)}${unit}`;
  };
  renderPointingTelemetry(fields.pointingLiveAzimuthPosition, azimuth?.position_deg, "°");
  renderPointingTelemetry(fields.pointingLiveAltitudePosition, altitude?.position_deg, "°");
  renderPointingTelemetry(fields.pointingLiveAzimuthVelocity, azimuth?.velocity_deg_per_sec, " °/s");
  renderPointingTelemetry(fields.pointingLiveAltitudeVelocity, altitude?.velocity_deg_per_sec, " °/s");
...
function setViewMode(mode, { persist = true } = {}) {
  ...
  if (activeViewMode !== "pointing" && fields.pointingCameraWindow && !fields.pointingCameraWindow.hidden) {
    closePointingCameraWindow();
  }
  if (
    activeViewMode !== "pointing"
    && (
      (fields.pointingAlignmentPanel && !fields.pointingAlignmentPanel.hidden)
      || (fields.pointingAlignmentMinimized && !fields.pointingAlignmentMinimized.hidden)
    )
  ) {
    closePointingAlignment({ reopenComparison: false });
  }
  if (activeViewMode !== "pointing" && fields.pointingModelEditor && !fields.pointingModelEditor.hidden) {
    closePointingModelEditor();
  }
...
function openSettingsPanel() {
  ...
  const requestId = ++settingsOpenRequestId;
  // Paint cached values immediately. Camera traffic and network latency must
  // not delay opening this local floating window.
  ...
  Promise.allSettled([
    loadAppSettings({ preserveOnError: true, populateOpenPanel: false }),
    loadSystemInfo({ quiet: true }),
  ]).then(() => {
    if (
      requestId === settingsOpenRequestId
      && !fields.settingsPanel.hidden
      && !settingsFormDirty
    ) {
      populateSettingsForm();
    }
...
function setSettingsActionMessage(message, state = "") {
  if (!fields.settingsActionMessage) return;
  fields.settingsActionMessage.textContent = message;
  fields.settingsActionMessage.classList.toggle("is-ok", state === "ok");
  fields.settingsActionMessage.classList.toggle("is-error", state === "error");
}

async function applySystemSettings() {
  addLog("message", "Applying temporary runtime settings");
  setSettingsActionMessage("Applying settings without resetting the mount...", "");
  if (fields.settingsApply) fields.settingsApply.disabled = true;
  if (fields.settingsSave) fields.settingsSave.disabled = true;
  try {
    const payload = settingsPayloadFromForm();
    payload.flash_drives = false;
    const response = await fetchWithTimeout("/api/settings/apply", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }, SETTINGS_REQUEST_TIMEOUT_MS);
    const data = await response.json();
    if (!response.ok || !data.ok) {
      throw new Error(data.error || "Settings apply failed.");
    }
    ...
    setSettingsActionMessage(
      "Applied for testing only — not saved, flashed, reconnected, or reset.",
      "ok",
    );
    addLog("success", "Temporary settings applied without a hardware reset");
    // The API response is the final Apply result. Optional view/status refreshes
    // run afterward and cannot leave the progress message hanging.
    if (activeViewMode === "pointing") void loadPointingModel({ force: true });
    if (!statusStreamConnected) void refreshStatus();
  } catch (error) {
    setSettingsActionMessage(error.message, "error");
    addLog("error", `System settings apply failed: ${error.message}`);
  } finally {
    if (fields.settingsApply) fields.settingsApply.disabled = false;
...
async function saveSystemSettings() {
  ...
  setSettingsActionMessage("Saving settings and reloading mount hardware...", "");
  if (fields.settingsApply) fields.settingsApply.disabled = true;
  if (fields.settingsSave) fields.settingsSave.disabled = true;
  try {
    ...
    setSettingsActionMessage("Settings saved; mount hardware configuration reloaded.", "ok");
  } catch (error) {
    setSettingsActionMessage(error.message, "error");
...
function updatePointingReadouts() {
  ...
  if (fields.pointingImageryAttribution) {
    const imagery = pointingModel.imagery || {};
    const textureSize = toNumber(imagery.texture_size_px);
    const groundResolution = toNumber(imagery.ground_resolution_m);
    const detail = textureSize !== null && groundResolution !== null
      ? `Satellite ${formatCompactNumber(textureSize)} px · ${formatNumber(groundResolution)} m/px · `
      : "";
    fields.pointingImageryAttribution.textContent = imagery.available ? `${detail}${imagery.attribution || ""}` : "";
  }
...
function renderPointingModel() {
  ...
  if (pointing3DReady && window.pointingTerrain3D) {
    ...
    window.pointingTerrain3D.setAlignmentPoints(appSettings.pointing_alignment?.points || []);
    window.pointingTerrain3D.showSelection(selectedPointingSample, { showLabel: true });
...
async function initializePointingTerrain3D() {
  ...
  try {
    pointing3DReady = await window.pointingTerrain3D.load(
      ...
      pointingModel,
      pointingLayerState(),
      appSettings.pointing_alignment?.points || [],
    );
...
function pointingLayerState() {
  return {
    ...
    visibility: fields.pointingLayerVisibility?.checked === true,
    grid: fields.pointingLayerGrid?.checked === true,
  };
}
...
async function selectPointingMapPoint(event) {
  ...
  if (
    event.currentTarget === fields.pointingMap3DCanvas
    && window.pointingTerrain3D?.consumeNavigationClick?.()
  ) return;
  const target = pointingTargetFromPointerEvent(event);
  if (!target) return;
  const requestSequence = ++pointingSampleRequestSequence;
  pointingSampleController?.abort();
  pointingSampleController = new AbortController();
  if (pointing3DReady && window.pointingTerrain3D && pointingModel?.station) {
    const earthRadiusM = 6371008.8;
    const stationLatitude = Number(pointingModel.station.latitude);
    const stationLongitude = Number(pointingModel.station.longitude);
    const latitudeDelta = (Number(target.latitude) - stationLatitude) * Math.PI / 180;
    const longitudeDelta = (Number(target.longitude) - stationLongitude) * Math.PI / 180;
    const meanLatitude = (Number(target.latitude) + stationLatitude) * Math.PI / 360;
    const distanceM = earthRadiusM * Math.hypot(
      latitudeDelta,
      longitudeDelta * Math.cos(meanLatitude),
    );
    window.pointingTerrain3D.showSelection({
  ...
  try {
    const sample = await fetchPointingSample(target, pointingSampleController.signal);
    if (activeViewMode !== "pointing" || requestSequence !== pointingSampleRequestSequence) return;
    selectedPointingSample = sample;
    updatePointingSelectionReadouts(selectedPointingSample);
    ...
    if (requestSequence === pointingSampleRequestSequence) pointingSampleController = null;
  }
}

function mountAzimuthForPointingSample(sample) {
  const normalized = toNumber(sample?.corrected_device_azimuth_deg)
    ?? toNumber(sample?.device_azimuth_deg);
...
function mountAltitudeForPointingSample(sample) {
  return toNumber(sample?.corrected_altitude_deg) ?? toNumber(sample?.altitude_deg);
}

function pointingModelAngleText(value) {
  const number = toNumber(value);
  return number === null ? "--" : `${formatSignedNumber(number)}°`;
}

function cameraZoomPercentage(metadata) {
  let percentage = Number(metadata?.zoom_pct);
  if (!Number.isFinite(percentage)) {
    const ratio = Number(metadata?.zoom_ratio);
    percentage = Number.isFinite(ratio) ? ratio * 100 : Number.NaN;
  }
...
function renderPointingModelZoomStatus() {
  if (pointingModelLinkedZoomPct === null) {
    pointingModelLinkedZoomPct = cameraZoomPercentage(cameraStreamMetadata.thermal)
      ?? cameraZoomPercentage(cameraStreamMetadata.visible);
  }
  [fields.pointingModelLinkedZoomPct, fields.pointingAlignmentLinkedZoomPct].forEach((input) => {
    if (!input) return;
    if (document.activeElement !== input) {
      input.value = pointingModelLinkedZoomPct === null
        ? ""
        : pointingModelLinkedZoomPct.toFixed(1);
    }
    input.placeholder = pointingModelLinkedZoomPct === null ? "--.-" : "";
  });
}

function setPointingModelZoomMessage(message, state = "") {
  [fields.pointingModelZoomMessage, fields.pointingAlignmentZoomMessage].forEach((element) => {
    if (!element) return;
    element.textContent = message;
...
function setPointingModelZoomBusy(busy) {
  pointingModelZoomInFlight = busy;
  [
    fields.pointingModelZoomOut,
    fields.pointingModelZoomIn,
    fields.pointingAlignmentZoomOut,
    fields.pointingAlignmentZoomIn,
  ].forEach((button) => {
    if (!button) return;
...
function pointingModelZoomStep(input = fields.pointingModelZoomStep) {
  const step = Number(input?.value);
  if (!Number.isFinite(step) || step < 0.1 || step > 25) {
    setPointingModelZoomMessage("Zoom Step must be between 0.1% and 25%.", "is-error");
    input?.focus();
    return null;
  }
  const normalized = Math.round(step * 10) / 10;
  [fields.pointingModelZoomStep, fields.pointingAlignmentZoomStep].forEach((candidate) => {
    if (candidate && candidate !== input) candidate.value = normalized.toFixed(1);
  });
  return normalized;
}

async function loadPointingModelLinkedZoom() {
  try {
    const response = await fetch("/api/camera-control/zoom", { cache: "no-store" });
    const result = await response.json().catch(() => ({}));
    const linked = Number(result.linked_zoom_pct);
    if (response.ok && result.ok === true && Number.isFinite(linked)) {
      pointingModelLinkedZoomPct = Math.max(0, Math.min(100, linked));
      renderPointingModelZoomStatus();
    }
  } catch (_error) {
    // Live frame metadata remains a safe read-only fallback for initial display.
  }
}

async function sendPointingModelZoom(direction, stepInput = fields.pointingModelZoomStep) {
  if (pointingModelZoomInFlight || !["in", "out"].includes(direction)) return;
  const step = pointingModelZoomStep(stepInput);
  if (step === null) return;
  setPointingModelZoomBusy(true);
  setPointingModelZoomMessage(`Applying ${step.toFixed(1)}% zoom ${direction} to both cameras…`);
  try {
    const response = await fetch("/api/camera-control/zoom", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ direction, step_pct: step }),
    });
    const result = await response.json().catch(() => ({}));
    if (!response.ok || result.ok !== true) {
      throw new Error(result.error || `Zoom command failed (${response.status})`);
    }
    const linked = Number(result.linked_zoom_pct);
    if (!Number.isFinite(linked)) throw new Error("Zoom API did not return a linked target.");
    pointingModelLinkedZoomPct = Math.max(0, Math.min(100, linked));
    renderPointingModelZoomStatus();
    setPointingModelZoomMessage(
      `Linked target ${pointingModelLinkedZoomPct.toFixed(1)}% · applied to Thermal and Visible`,
      "is-ok",
    );
  } catch (error) {
    setPointingModelZoomMessage(error.message || "Zoom command failed.", "is-error");
  } finally {
    setPointingModelZoomBusy(false);
  }
}

async function commitPointingModelLinkedZoom(input = fields.pointingModelLinkedZoomPct) {
  if (!input || pointingModelZoomInFlight) return;
  const rawTarget = input.value.trim();
  const target = Number(rawTarget);
  if (!rawTarget || !Number.isFinite(target) || target < 0 || target > 100) {
    setPointingModelZoomMessage("Linked Zoom must be between 0% and 100%.", "is-error");
    renderPointingModelZoomStatus();
    return;
  }
  const normalizedTarget = Math.round(target * 10) / 10;
  if (
    pointingModelLinkedZoomPct !== null
    && Math.abs(normalizedTarget - pointingModelLinkedZoomPct) < 0.05
  ) {
    renderPointingModelZoomStatus();
    return;
  }
  ...
  setPointingModelZoomBusy(true);
  setPointingModelZoomMessage(`Applying linked zoom ${normalizedTarget.toFixed(1)}%…`);
  try {
    const response = await fetch("/api/camera-control/zoom", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ target_zoom_pct: normalizedTarget }),
    });
    const result = await response.json().catch(() => ({}));
    if (!response.ok || result.ok !== true) {
      throw new Error(result.error || `Zoom command failed (${response.status})`);
...
function bindPointingModelZoomButton(button, direction, stepInput = fields.pointingModelZoomStep) {
  if (!button) return;
  let holdState = null;
  let suppressNextClick = false;
  let suppressResetTimer = null;

  const repeatHeldStep = async (state) => {
    if (!state.active) return;
    await sendPointingModelZoom(direction, stepInput);
    if (state.active) state.timer = setTimeout(() => repeatHeldStep(state), 350);
  };
  const stopHold = () => {
    if (!holdState) return;
    holdState.active = false;
    clearTimeout(holdState.timer);
    if (holdState.repeating) {
  ...
  button.addEventListener("pointerdown", (event) => {
    ...
    stopHold();
    const state = { active: true, repeating: false, timer: null };
    holdState = state;
    button.setPointerCapture?.(event.pointerId);
    state.timer = setTimeout(() => {
      if (!state.active) return;
      state.repeating = true;
      repeatHeldStep(state);
    }, 450);
  });
  ...
  button.addEventListener("click", (event) => {
    ...
    sendPointingModelZoom(direction, stepInput);
  });
  button.addEventListener("contextmenu", (event) => event.preventDefault());
}

function renderPointingModelComparison() {
  const sample = pointingModelComparisonSample;
  if (!sample) return;
  const axes = axesByLabel();
  const azimuthAxis = latestStatus?.connected && axes.Azimuth?.available ? axes.Azimuth : null;
  const altitudeAxis = latestStatus?.connected && axes.Altitude?.available ? axes.Altitude : null;
  const demAzimuth = mountAzimuthForPointingSample(sample);
  const demAltitude = mountAltitudeForPointingSample(sample);
  const telemetryAzimuth = toNumber(azimuthAxis?.position_deg);
  const telemetryAltitude = toNumber(altitudeAxis?.position_deg);
  const azimuthDifference = demAzimuth === null || telemetryAzimuth === null
    ? null
    : normalizeSignedDegrees(demAzimuth - telemetryAzimuth);
  const altitudeDifference = demAltitude === null || telemetryAltitude === null
    ? null
    : demAltitude - telemetryAltitude;

  if (fields.pointingModelComparisonGrid) {
    fields.pointingModelComparisonGrid.textContent = sample.grid_cell?.id || "30 m terrain cell";
  }
  if (fields.pointingModelLatitude) fields.pointingModelLatitude.textContent = formatCoordinate(sample.latitude);
  if (fields.pointingModelLongitude) fields.pointingModelLongitude.textContent = formatCoordinate(sample.longitude);
  if (fields.pointingModelDemAzimuth) fields.pointingModelDemAzimuth.textContent = pointingModelAngleText(demAzimuth);
  if (fields.pointingModelDemAltitude) fields.pointingModelDemAltitude.textContent = pointingModelAngleText(demAltitude);
  if (fields.pointingModelTelemetryAzimuth) fields.pointingModelTelemetryAzimuth.textContent = pointingModelAngleText(telemetryAzimuth);
  if (fields.pointingModelTelemetryAltitude) fields.pointingModelTelemetryAltitude.textContent = pointingModelAngleText(telemetryAltitude);
  if (fields.pointingModelAzimuthDifference) fields.pointingModelAzimuthDifference.textContent = pointingModelAngleText(azimuthDifference);
  if (fields.pointingModelAltitudeDifference) fields.pointingModelAltitudeDifference.textContent = pointingModelAngleText(altitudeDifference);
  renderPointingModelZoomStatus();
  if (fields.pointingModelComparisonMessage) {
    fields.pointingModelComparisonMessage.textContent = telemetryAzimuth === null || telemetryAltitude === null
      ? "Waiting for live mount telemetry. DEM azimuth is expressed in mount coordinates."
      : "Difference = DEM target − mount telemetry. DEM azimuth uses mount coordinates; telemetry updates live.";
  }
}

function openPointingModelComparison() {
  const panel = fields.pointingModelComparisonPanel;
  if (!panel || !selectedPointingSample || activeViewMode !== "pointing") return;
  pointingModelComparisonSample = { ...selectedPointingSample };
  panel.hidden = false;
  setPointingModelZoomMessage("Click once for one step. Press and hold for continuous steps.");
  renderPointingModelComparison();
  loadPointingModelLinkedZoom();
  const labelRect = document.getElementById("pointingHoverLabel")?.getBoundingClientRect();
  const candidateLeft = labelRect
    ? Math.min(window.innerWidth - panel.offsetWidth - 20, labelRect.right + 16)
    : Math.max(220, (window.innerWidth - panel.offsetWidth) / 2);
  restoreFloatingWindowPosition(panel, {
    left: Math.max(20, candidateLeft),
    top: labelRect ? Math.max(86, labelRect.top) : 150,
  });
}

function setPointingAlignmentMessage(message, state = "") {
  const element = fields.pointingAlignmentMessage || fields.pointingAlignmentDirectionMessage;
  if (!element) return;
  element.textContent = message;
...
function pointingAlignmentDirectionMode() {
  return fields.pointingAlignmentDirectionMode?.value === "jog" ? "jog" : "offset";
}

function updatePointingAlignmentDirectionMode() {
  const nextMode = pointingAlignmentDirectionMode();
  if (fields.pointingAlignmentOffsetStep) {
    pointingAlignmentDirectionValues[activePointingAlignmentDirectionMode] = fields.pointingAlignmentOffsetStep.value;
  }
  if (activeDirectionJog) stopActiveDirectionJog(true, false);
  activePointingAlignmentDirectionMode = nextMode;
  const offsetMode = nextMode === "offset";
  if (fields.pointingAlignmentDirectionValueLabel) {
    fields.pointingAlignmentDirectionValueLabel.textContent = offsetMode ? "Movement Offset" : "Movement Speed";
  }
  if (fields.pointingAlignmentDirectionUnit) {
    fields.pointingAlignmentDirectionUnit.textContent = offsetMode ? "Deg" : "Deg/Sec";
  }
  if (fields.pointingAlignmentOffsetStep) {
    fields.pointingAlignmentOffsetStep.step = offsetMode ? "0.001" : "0.1";
    fields.pointingAlignmentOffsetStep.value = pointingAlignmentDirectionValues[nextMode];
    fields.pointingAlignmentOffsetStep.setAttribute(
      "aria-label",
      offsetMode ? "Movement offset in degrees" : "Movement speed in degrees per second",
    );
  }
  if (fields.pointingAlignmentDirectionPad) {
    fields.pointingAlignmentDirectionPad.setAttribute(
      "aria-label",
      offsetMode ? "Click a direction to move by the configured offset" : "Press and hold a direction to move",
    );
  }
  setPointingAlignmentMessage(
    offsetMode ? "Click a direction to move by the configured offset." : "Press and hold to move • Release to stop.",
  );
}

function pointingAlignmentValues() {
  const sample = pointingAlignmentSample;
  if (!sample) return null;
  const axes = axesByLabel();
  const demAzimuth = mountAzimuthForPointingSample(sample);
  const demAltitude = mountAltitudeForPointingSample(sample);
  const telemetryAzimuth = latestStatus?.connected && axes.Azimuth?.available
    ? toNumber(axes.Azimuth.position_deg)
    : null;
  const telemetryAltitude = latestStatus?.connected && axes.Altitude?.available
    ? toNumber(axes.Altitude.position_deg)
    : null;
  const rawTelemetryAzimuth = latestStatus?.connected && axes.Azimuth?.available
    ? toNumber(axes.Azimuth.raw_position_deg)
    : null;
  const rawTelemetryAltitude = latestStatus?.connected && axes.Altitude?.available
    ? toNumber(axes.Altitude.raw_position_deg)
    : null;
  const adjustedAzimuth = demAzimuth === null ? null : demAzimuth + pointingAlignmentOffset.azimuth;
  const adjustedAltitude = demAltitude === null ? null : demAltitude + pointingAlignmentOffset.altitude;
  return {
    demAzimuth,
    demAltitude,
    adjustedAzimuth,
    ...
    telemetryAltitude,
    rawTelemetryAzimuth,
    rawTelemetryAltitude,
    residualAzimuth: adjustedAzimuth === null || telemetryAzimuth === null
      ? null
      : normalizeSignedDegrees(adjustedAzimuth - telemetryAzimuth),
    residualAltitude: adjustedAltitude === null || telemetryAltitude === null
      ? null
      : adjustedAltitude - telemetryAltitude,
  };
}

function pointingModelPointSample(point) {
  return {
    ...point,
    grid_cell: { id: point.grid_cell || "Saved terrain reference" },
    device_azimuth_deg: point.reference_azimuth_deg,
    altitude_deg: point.reference_altitude_deg,
  };
}

function focusPointingModelPoint(point, row = null) {
  if (!point || !window.pointingTerrain3D) return;
  const focused = window.pointingTerrain3D.focusAlignmentPoint(point);
  if (!focused) return;
  selectedPointingModelPointId = String(point.id || "");
  fields.pointingModelEditorBody?.querySelectorAll("tr").forEach((candidate) => {
    const active = candidate.dataset.pointId === selectedPointingModelPointId;
...
function renderPointingModelEditor() {
  const alignment = appSettings.pointing_alignment || {};
  const points = Array.isArray(alignment.points) ? alignment.points : [];
  const model = alignment.model || {};
  if (fields.pointingModelEditorCount) fields.pointingModelEditorCount.textContent = String(points.length);
  if (fields.pointingModelEditorStatus) {
    fields.pointingModelEditorStatus.textContent = `${points.length} POINT${points.length === 1 ? "" : "S"}`;
  }
  if (fields.pointingModelEditorModelState) {
    const rms = toNumber(model.rms_error_deg);
    fields.pointingModelEditorModelState.textContent = model.active
      ? `MODEL ACTIVE · RMS ${rms === null ? "--" : rms.toFixed(3)}°`
      : `MODEL INACTIVE · ${points.length}/${Number(model.required_point_count) || 2} REQUIRED`;
    fields.pointingModelEditorModelState.classList.toggle("is-active", model.active === true);
  }
  const body = fields.pointingModelEditorBody;
  if (!body) return;
  body.replaceChildren();
  if (fields.pointingModelEditorEmpty) fields.pointingModelEditorEmpty.hidden = points.length > 0;
  points.slice().reverse().forEach((point, reverseIndex) => {
    const row = document.createElement("tr");
    const pointNumber = points.length - reverseIndex;
    const savedAt = new Date(point.saved_at);
    const savedLabel = Number.isNaN(savedAt.getTime())
      ? (point.saved_at || "--")
      : savedAt.toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" });
    const distanceKm = toNumber(point.distance_m);
    row.tabIndex = 0;
    row.dataset.pointId = String(point.id || "");
    row.setAttribute("role", "button");
    row.setAttribute("aria-selected", row.dataset.pointId === selectedPointingModelPointId ? "true" : "false");
    row.classList.toggle("is-active", row.dataset.pointId === selectedPointingModelPointId);
    const values = [
      null,
      formatCoordinate(point.latitude),
      formatCoordinate(point.longitude),
      `${pointingAlignmentAngleText(point.reference_azimuth_deg)} / ${pointingAlignmentAngleText(point.reference_altitude_deg)}`,
      `${pointingAlignmentAngleText(point.telemetry_azimuth_deg)} / ${pointingAlignmentAngleText(point.telemetry_altitude_deg)}`,
      distanceKm === null ? "--" : `${(distanceKm / 1000).toFixed(2)} km`,
      savedLabel,
    ];
    values.forEach((value, index) => {
      ...
      if (index === 0) {
        ...
        number.textContent = `P${String(pointNumber).padStart(2, "0")}`;
        grid.textContent = point.grid_cell || "Terrain reference";
        cell.append(number, grid);
      } else {
    ...
    remove.setAttribute("aria-label", `Delete ${point.grid_cell || `point ${pointNumber}`}`);
    remove.addEventListener("click", (event) => {
      event.stopPropagation();
      deletePointingAlignmentPoint(point.id, remove);
    });
    actionCell.append(remove);
    row.append(actionCell);
    const focus = () => focusPointingModelPoint(pointingModelPointSample(point), row);
    row.addEventListener("click", (event) => {
      if (!event.target.closest("button")) focus();
    });
...
function openPointingModelEditor() {
  const panel = fields.pointingModelEditor;
  if (!panel || activeViewMode !== "pointing") return;
  renderPointingModelEditor();
  panel.hidden = false;
  fields.pointingModelEditorOpen?.setAttribute("aria-expanded", "true");
  restoreFloatingWindowPosition(panel, {
    left: Math.max(20, (window.innerWidth - panel.offsetWidth) / 2),
    top: Math.max(90, (window.innerHeight - panel.offsetHeight) / 2),
  });
}

function closePointingModelEditor() {
  if (!fields.pointingModelEditor) return;
  fields.pointingModelEditor.hidden = true;
  fields.pointingModelEditorOpen?.setAttribute("aria-expanded", "false");
}

function renderPointingAlignmentModel() {
  const alignment = appSettings.pointing_alignment || {};
  const points = Array.isArray(alignment.points) ? alignment.points : [];
  const model = alignment.model || {};
  if (selectedPointingModelPointId && !points.some((point) => String(point.id || "") === selectedPointingModelPointId)) {
    selectedPointingModelPointId = "";
  }
  window.pointingTerrain3D?.setAlignmentPoints(points);
  renderPointingModelEditor();
  if (fields.pointingAlignmentPointCount) {
    fields.pointingAlignmentPointCount.textContent = `${points.length} POINT${points.length === 1 ? "" : "S"}`;
  }
  if (fields.pointingAlignmentModelError) {
    const rms = toNumber(model.rms_error_deg);
    fields.pointingAlignmentModelError.textContent = model.active
      ? `ACTIVE · RMS ${rms === null ? "--" : rms.toFixed(3)}°`
      : "INACTIVE";
    fields.pointingAlignmentModelError.classList.toggle("is-active", model.active === true);
  }
  if (fields.pointingAlignmentMinimizedStatus) {
    fields.pointingAlignmentMinimizedStatus.textContent = `${points.length} POINT${points.length === 1 ? "" : "S"} · ${model.active ? "ACTIVE" : "INACTIVE"}`;
  }
}

async function deletePointingAlignmentPoint(pointId, button) {
  if (!pointId || !window.confirm("Delete this mount calibration point and rebuild the model?")) return;
  button.disabled = true;
  try {
    const response = await fetch(`/api/pointing/alignment-point/${encodeURIComponent(pointId)}`, { method: "DELETE" });
    const data = await response.json();
    if (!response.ok || !data.ok) throw new Error(data.error || "Could not delete calibration point.");
    appSettings = data.settings;
    renderPointingAlignmentModel();
    setPointingAlignmentMessage("Calibration point deleted. The mount model was rebuilt from the remaining points.", "is-ok");
  } catch (error) {
    button.disabled = false;
    setPointingAlignmentMessage(error.message, "is-error");
  }
}

function pointingAlignmentAngleText(value, forceSign = false) {
  const number = toNumber(value);
  if (number === null) return "--";
  const formatted = Math.abs(number).toFixed(3);
  if (forceSign || number < 0) return `${number >= 0 ? "+" : "−"}${formatted}°`;
  return `${formatted}°`;
}

function renderPointingAlignment() {
  const values = pointingAlignmentValues();
  if (!values) return;
  if (fields.pointingAlignmentGrid) {
    fields.pointingAlignmentGrid.textContent = pointingAlignmentSample.grid_cell?.id || "30 m terrain cell";
  }
  const mappings = [
    [fields.pointingAlignmentDemAzimuth, values.demAzimuth],
    [fields.pointingAlignmentDemAltitude, values.demAltitude],
    [fields.pointingAlignmentAdjustedAzimuth, values.adjustedAzimuth],
    [fields.pointingAlignmentAdjustedAltitude, values.adjustedAltitude],
    [fields.pointingAlignmentTelemetryAzimuth, values.telemetryAzimuth],
    [fields.pointingAlignmentTelemetryAltitude, values.telemetryAltitude],
    [fields.pointingAlignmentRawAzimuth, values.rawTelemetryAzimuth],
    [fields.pointingAlignmentRawAltitude, values.rawTelemetryAltitude],
  ];
  mappings.forEach(([element, value]) => {
    if (element) element.textContent = pointingAlignmentAngleText(value);
  });
  if (fields.pointingAlignmentOffsetAzimuth) {
    fields.pointingAlignmentOffsetAzimuth.textContent = pointingAlignmentAngleText(pointingAlignmentOffset.azimuth, true);
  }
  if (fields.pointingAlignmentOffsetAltitude) {
    fields.pointingAlignmentOffsetAltitude.textContent = pointingAlignmentAngleText(pointingAlignmentOffset.altitude, true);
  }
  if (fields.pointingAlignmentResidualAzimuth) {
    fields.pointingAlignmentResidualAzimuth.textContent = pointingAlignmentAngleText(values.residualAzimuth, true);
  }
  if (fields.pointingAlignmentResidualAltitude) {
    fields.pointingAlignmentResidualAltitude.textContent = pointingAlignmentAngleText(values.residualAltitude, true);
  }
  renderPointingModelZoomStatus();
  renderPointingAlignmentModel();
}

function releasePointingAlignmentStreams() {
  pointingAlignmentFrameSession += 1;
  pointingAlignmentFrameTimers.forEach((timer) => clearTimeout(timer));
  pointingAlignmentFrameTimers.clear();
  [fields.pointingAlignmentThermalImage, fields.pointingAlignmentVisibleImage].forEach((image) => {
    if (!image) return;
    image.onload = null;
    image.onerror = null;
...
function attachPointingAlignmentLatestFrame(image, camera, session) {
  if (
    !image
    || session !== pointingAlignmentFrameSession
    || fields.pointingAlignmentPanel?.hidden
    || document.hidden
  ) return;
  const startedAt = performance.now();
  const scheduleNext = (failed = false) => {
    if (session !== pointingAlignmentFrameSession || fields.pointingAlignmentPanel?.hidden) return;
    const delay = Math.max(0, (failed ? 500 : pointingCameraFrameIntervalMs) - (performance.now() - startedAt));
    pointingAlignmentFrameTimers.set(camera, setTimeout(
      () => attachPointingAlignmentLatestFrame(image, camera, session),
      delay,
    ));
  };
  image.onload = () => {
    if (session !== pointingAlignmentFrameSession) return;
  ...
  image.src = `/api/camera-frame/latest/live/${camera}?preview=pointing&alignment=${session}&_=${Date.now()}`;
}

function startPointingAlignmentStreams() {
  releasePointingAlignmentStreams();
  const session = ++pointingAlignmentFrameSession;
  attachPointingAlignmentLatestFrame(fields.pointingAlignmentThermalImage, "thermal", session);
  attachPointingAlignmentLatestFrame(fields.pointingAlignmentVisibleImage, "visible", session);
}

function pointingAlignmentStreamsNeedReconnect() {
  return [fields.pointingAlignmentThermalImage, fields.pointingAlignmentVisibleImage]
    .some((image) => image && (!image.getAttribute("src") || image.classList.contains("is-error")));
}

function openPointingAlignment() {
  const panel = fields.pointingAlignmentPanel;
  const sourceSample = pointingModelComparisonSample || selectedPointingSample;
  if (!panel || !sourceSample || activeViewMode !== "pointing") return;
  pointingAlignmentSample = { ...sourceSample };
  pointingAlignmentOffset = { azimuth: 0, altitude: 0 };
  if (fields.pointingModelComparisonPanel) fields.pointingModelComparisonPanel.hidden = true;
  if (fields.pointingCameraWindow && !fields.pointingCameraWindow.hidden) closePointingCameraWindow();
  if (fields.pointingAlignmentMinimized) fields.pointingAlignmentMinimized.hidden = true;
  panel.hidden = false;
  updatePointingAlignmentDirectionMode();
  renderPointingAlignment();
  loadPointingModelLinkedZoom();
  restoreFloatingWindowPosition(panel, {
    left: Math.max(20, (window.innerWidth - panel.offsetWidth) / 2),
    top: Math.max(24, (window.innerHeight - panel.offsetHeight) / 2),
  });
  startPointingAlignmentStreams();
}

function minimizePointingAlignment() {
  if (!fields.pointingAlignmentPanel || fields.pointingAlignmentPanel.hidden) return;
  if (activeDirectionJog) stopActiveDirectionJog();
  releasePointingAlignmentStreams();
  fields.pointingAlignmentPanel.hidden = true;
  if (fields.pointingAlignmentMinimized) fields.pointingAlignmentMinimized.hidden = false;
  renderPointingAlignmentModel();
}

function restorePointingAlignment() {
  const panel = fields.pointingAlignmentPanel;
  if (!panel || !pointingAlignmentSample || activeViewMode !== "pointing") return;
  if (fields.pointingAlignmentMinimized) fields.pointingAlignmentMinimized.hidden = true;
  panel.hidden = false;
  renderPointingAlignment();
  loadPointingModelLinkedZoom();
...
function closePointingAlignment({ reopenComparison = true } = {}) {
  if (!fields.pointingAlignmentPanel) return;
  if (activeDirectionJog) stopActiveDirectionJog();
  releasePointingAlignmentStreams();
  fields.pointingAlignmentPanel.hidden = true;
  if (fields.pointingAlignmentMinimized) fields.pointingAlignmentMinimized.hidden = true;
  if (
    reopenComparison
    && fields.pointingModelComparisonPanel
    && pointingModelComparisonSample
    && activeViewMode === "pointing"
  ) {
    fields.pointingModelComparisonPanel.hidden = false;
    renderPointingModelComparison();
  }
}

function applyPointingAlignmentAcceptedOffset({ amount, azimuthSign, altitudeSign }) {
  const divisor = Math.hypot(azimuthSign, altitudeSign) || 1;
  pointingAlignmentOffset.azimuth += (amount * azimuthSign) / divisor;
  pointingAlignmentOffset.altitude += (amount * altitudeSign) / divisor;
  pointingAlignmentOffset.azimuth = Math.round(pointingAlignmentOffset.azimuth * 1000) / 1000;
  pointingAlignmentOffset.altitude = Math.round(pointingAlignmentOffset.altitude * 1000) / 1000;
  renderPointingAlignment();
}

async function addPointingAlignmentCalculationPoint() {
  const values = pointingAlignmentValues();
  const sample = pointingAlignmentSample;
  if (
    !values || !sample
    || values.adjustedAzimuth === null || values.adjustedAltitude === null
    || values.telemetryAzimuth === null || values.telemetryAltitude === null
  ) {
    setPointingAlignmentMessage("Live DEM and mount telemetry values are required before calculating offsets.", "is-error");
    return;
  }
  const confirmed = window.confirm(
    `Add ${sample.grid_cell?.id || "this terrain location"} to the mount compensation model?\n\n`
    + `Reference: ${pointingAlignmentAngleText(toNumber(sample.device_azimuth_deg))} / ${pointingAlignmentAngleText(toNumber(sample.altitude_deg))}\n`
    + `Observed mount: ${pointingAlignmentAngleText(values.telemetryAzimuth)} / ${pointingAlignmentAngleText(values.telemetryAltitude)}\n\n`
    + "The mount will not move and encoder coordinates will not be rebased.",
  );
  if (!confirmed) {
    setPointingAlignmentMessage("Calculation point was not applied.", "is-warning");
    return;
  }
  const payload = {
    grid_cell: String(sample.grid_cell?.id || ""),
    latitude: Number(sample.latitude),
    longitude: Number(sample.longitude),
    camera_offset_azimuth_deg: pointingAlignmentOffset.azimuth,
    camera_offset_altitude_deg: pointingAlignmentOffset.altitude,
  };
  fields.pointingAlignmentSave.disabled = true;
  let pointSaved = false;
  setPointingAlignmentMessage("Saving point and rebuilding the three-axis mount model…");
  try {
    const response = await fetch("/api/pointing/alignment-point", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    const data = await response.json();
    if (!response.ok || !data.ok) throw new Error(data.error || "Could not add alignment calculation point.");
    appSettings = data.settings;
    const model = data.model || {};
    renderPointingAlignmentModel();
    setPointingAlignmentMessage(
      model.active
        ? `Point saved. Mount compensation is active from ${model.point_count} points (RMS error ${Number(model.rms_error_deg).toFixed(3)}°).`
        : `Point saved (${model.point_count || 1}/2 minimum). Add a well-separated point to activate compensation.`,
      "is-ok",
    );
    addLog("success", `Mount calibration point saved for ${payload.grid_cell || "selected DEM point"}`);
    pointSaved = true;
  } catch (error) {
    setPointingAlignmentMessage(error.message, "is-error");
  } finally {
    fields.pointingAlignmentSave.disabled = false;
  }
  if (pointSaved) {
    closePointingAlignment({ reopenComparison: false });
    openPointingModelEditor();
  }
}
...
function openPointingTargetPanel(sample) {
  ...
  const mountAltitude = mountAltitudeForPointingSample(sample);
  panel.hidden = false;
  ...
  if (fields.pointingTargetSlantRange) fields.pointingTargetSlantRange.textContent = sample.slant_range_m === null ? "--" : `${formatNumber(Number(sample.slant_range_m) / 1000)} km`;
  ...
  if (fields.pointingTargetAltitude) fields.pointingTargetAltitude.textContent = mountAltitude === null ? "--" : `${formatNumber(mountAltitude)} Deg`;
  if (fields.pointingTargetCurvature) fields.pointingTargetCurvature.textContent = sample.curvature_drop_m === null ? "--" : `${formatCompactNumber(sample.curvature_drop_m)} m drop`;
  if (fields.pointingTargetAlignment) {
    const pointCount = Number(sample.alignment_model?.point_count || 0);
    fields.pointingTargetAlignment.textContent = sample.alignment_applied
      ? `Active · ${pointCount} points`
      : pointCount ? `Inactive · ${pointCount} points` : "Not calibrated";
  }
  ...
  if (fields.pointingTargetGoto) {
    fields.pointingTargetGoto.disabled = mountAzimuth === null || mountAltitude === null || sample.elevation_m === null;
    fields.pointingTargetGoto.dataset.azimuth = mountAzimuth === null ? "" : String(mountAzimuth);
    fields.pointingTargetGoto.dataset.altitude = mountAltitude === null ? "" : String(mountAltitude);
  }
}
...
function directionControlMode() {
  return fields.directionMode?.value === "offset" ? "offset" : "jog";
}

function updateDirectionControlMode() {
  const nextMode = directionControlMode();
  if (fields.directionSpeed) {
    directionModeValues[activeDirectionMode] = fields.directionSpeed.value;
  }
  if (activeDirectionJog) stopActiveDirectionJog(true, false);
  activeDirectionMode = nextMode;

  const offsetMode = nextMode === "offset";
  if (fields.directionKicker) fields.directionKicker.textContent = offsetMode ? "MOUNT OFFSET" : "MOUNT JOG";
  if (fields.directionValueLabel) fields.directionValueLabel.textContent = offsetMode ? "Movement Offset" : "Movement Speed";
  if (fields.directionUnit) fields.directionUnit.textContent = offsetMode ? "Deg" : "Deg/Sec";
  if (fields.directionSpeed) {
    fields.directionSpeed.step = offsetMode ? "0.001" : "0.1";
    fields.directionSpeed.value = directionModeValues[nextMode];
    fields.directionSpeed.setAttribute("aria-label", offsetMode ? "Movement offset in degrees" : "Movement speed in degrees per second");
  }
  if (fields.directionPad) {
    fields.directionPad.setAttribute("aria-label", offsetMode
      ? "Click a direction to move by the configured offset"
      : "Press and hold a direction to move");
  }
  if (fields.directionSafety) {
    fields.directionSafety.textContent = offsetMode
      ? "Diagonal offset is normalized. Axis and runtime limits remain enforced."
      : "Diagonal speed is normalized. Axis and runtime limits remain enforced.";
  }
  setDirectionControlMessage(offsetMode
    ? "Click a direction to move by the configured offset"
    : "Press and hold to move • Release to stop");
}

function setPointingCameraDirectionMessage(message, className = "") {
  const element = fields.pointingCameraDirectionMessage;
  if (!element) return;
  element.classList.remove("is-ok", "is-error", "is-warning");
  if (className) element.classList.add(className);
  element.textContent = message;
...
function pointingCameraDirectionMode() {
  return fields.pointingCameraDirectionMode?.value === "offset" ? "offset" : "jog";
}

function updatePointingCameraDirectionMode() {
  const nextMode = pointingCameraDirectionMode();
  if (fields.pointingCameraDirectionValue) {
    pointingCameraDirectionValues[activePointingCameraDirectionMode] = fields.pointingCameraDirectionValue.value;
  }
  if (activeDirectionJog) stopActiveDirectionJog(true, false);
  activePointingCameraDirectionMode = nextMode;
  const offsetMode = nextMode === "offset";
  if (fields.pointingCameraDirectionValueLabel) {
    fields.pointingCameraDirectionValueLabel.textContent = offsetMode ? "Movement Offset" : "Movement Speed";
  }
  if (fields.pointingCameraDirectionUnit) {
    fields.pointingCameraDirectionUnit.textContent = offsetMode ? "Deg" : "Deg/Sec";
  }
  if (fields.pointingCameraDirectionValue) {
    fields.pointingCameraDirectionValue.step = offsetMode ? "0.001" : "0.1";
    fields.pointingCameraDirectionValue.value = pointingCameraDirectionValues[nextMode];
  }
  if (fields.pointingCameraDirectionPad) {
    fields.pointingCameraDirectionPad.setAttribute("aria-label", offsetMode
      ? "Click a direction to move by the configured offset"
      : "Press and hold a direction to move");
  }
  setPointingCameraDirectionMessage(offsetMode
    ? "Click a direction to move by the configured offset"
    : "Press and hold to move • Release to stop");
}

function calculateDirectionOffsetTargets(azimuth, altitude, amount, azimuthSign, altitudeSign) {
  const divisor = Math.hypot(azimuthSign, altitudeSign) || 1;
  const roundToThree = (value) => Math.round((value + Number.EPSILON) * 1000) / 1000;
  return {
    Azimuth: String(roundToThree(azimuth + ((amount * azimuthSign) / divisor))),
    Altitude: String(roundToThree(altitude + ((amount * altitudeSign) / divisor))),
  };
}

async function sendDirectionOffsetCommand(button, controls = {}) {
  const input = controls.input || fields.directionSpeed;
  const setMessage = controls.setMessage || setDirectionControlMessage;
  const amount = toNumber(input?.value);
  if (amount === null || amount <= 0) {
    setMessage("Movement offset must be greater than 0.", "is-error");
    return;
  }
  if (motorCommandInFlight) {
    setMessage("Wait for the current mount command to finish.", "is-warning");
    return;
  }

  const axes = axesByLabel();
  const currentAzimuth = toNumber(axes.Azimuth?.position_deg);
  const currentAltitude = toNumber(axes.Altitude?.position_deg);
  if (currentAzimuth === null || currentAltitude === null) {
    setMessage("Current mount position is unavailable.", "is-warning");
    return;
  }

  const azimuthSign = Number(
    button.dataset.directionAz
    ?? button.dataset.cameraDirectionAz
    ?? button.dataset.alignmentDirectionAz,
  );
  const altitudeSign = Number(
    button.dataset.directionAlt
    ?? button.dataset.cameraDirectionAlt
    ?? button.dataset.alignmentDirectionAlt,
  );
  const payload = calculateDirectionOffsetTargets(currentAzimuth, currentAltitude, amount, azimuthSign, altitudeSign);
  payload.position_tolerance_deg = String(Math.max(0.0005, Math.min(0.08, amount / 4)));
  const guardResult = motionCommandGuard.validate("goto", payload);
  if (!guardResult.ok) {
    setMessage(guardResult.message, "is-warning");
    return;
  }
  ...
  renderAxisControls(latestStatus);
  try {
    const response = await fetch("/api/motors/goto", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    const data = await response.json();
    if (!response.ok || !data.ok) throw new Error(data.error || "Direction offset command failed.");
    if (data.status) renderStatus(data.status);
    const direction = button.getAttribute("aria-label").replace("Move ", "");
    setMessage(`Offset ${direction} by ${amount.toFixed(3)} Deg`, "is-ok");
    controls.onAccepted?.({ amount, azimuthSign, altitudeSign, payload, data });
    addLog("message", `Direction offset ${direction}: ${amount.toFixed(3)} Deg`);
  } catch (error) {
    setMessage(error.message, "is-error");
    addLog("error", `Direction offset failed: ${error.message}`);
  } finally {
    button.classList.remove("is-pressed");
    motorCommandInFlight = false;
    renderMotorControls(latestStatus);
...
async function sendDirectionJogCommand(
  azimuth,
  altitude,
  message,
  showSuccessMessage = true,
  setMessage = setDirectionControlMessage,
) {
  ...
  if (!limitResult.ok) {
    setMessage(limitResult.message, "is-warning");
    return false;
  }
  ...
  try {
    ...
    if (showSuccessMessage) {
      setMessage(message, azimuth === 0 && altitude === 0 ? "is-ok" : "");
    }
    return true;
  } catch (error) {
    setMessage(error.message, "is-error");
...
function stopActiveDirectionJog(sendStop = true, showSuccessMessage = true) {
  ...
  state.button.classList.remove("is-pressed");
  if (sendStop) {
    sendDirectionJogCommand(
      0,
      0,
      "Direction jog stopped",
      showSuccessMessage,
      state.setMessage || setDirectionControlMessage,
    );
  }
}

function startDirectionJog(button, event, controls = {}) {
  const input = controls.input || fields.directionSpeed;
  const setMessage = controls.setMessage || setDirectionControlMessage;
  const speed = toNumber(input?.value);
  if (speed === null || speed <= 0) {
    setMessage("Movement speed must be greater than 0.", "is-error");
    return;
  }
  const azimuthSign = Number(
    button.dataset.directionAz
  ...
  stopActiveDirectionJog();
  const state = { button, heartbeatTimer: null, heartbeatInFlight: false, cleanup: null, setMessage };
  ...
  sendDirectionJogCommand(
    azimuth,
    altitude,
    `Moving ${button.getAttribute("aria-label").replace("Move ", "")}`,
    true,
    setMessage,
  );
...
async function loadAppSettings(options = {}) {
  try {
    ...
    applyTuningSteps();
    if (
      options.populateOpenPanel !== false
      && fields.settingsPanel
      && !fields.settingsPanel.hidden
    ) populateSettingsForm();
    return appSettings;
  } catch (error) {
...
let cameraStreamSession = 0;
let cameraStreamsWanted = false;
const cameraStreamReady = new Set();
const cameraStreamRetryTimers = new Map();
let cameraCalibrationAnimationFrame = null;
let cameraCalibrationLastRenderAt = 0;
const pointingCameraMetadataStreams = new Map();
const pointingCameraFrameTimers = new Map();
let pointingCameraFrameSession = 0;
let pointingCameraFrameIntervalMs = 100;

function releasePointingCameraStreams() {
  pointingCameraFrameSession += 1;
  pointingCameraFrameTimers.forEach((timer) => clearTimeout(timer));
  pointingCameraFrameTimers.clear();
  pointingCameraMetadataStreams.forEach((stream) => stream.close());
  pointingCameraMetadataStreams.clear();
  [fields.pointingCameraThermalZoomPct, fields.pointingCameraVisibleZoomPct].forEach((element) => {
    if (!element) return;
    element.textContent = "ZOOM --.-%";
    element.removeAttribute("title");
  });
  [fields.pointingCameraThermalImage, fields.pointingCameraVisibleImage].forEach((image) => {
    if (!image) return;
    image.onload = null;
    image.onerror = null;
    image.removeAttribute("src");
...
function attachPointingLatestFrame(image, camera, session) {
  if (
    !image
    || session !== pointingCameraFrameSession
    || fields.pointingCameraWindow?.hidden
    || document.hidden
  ) return;
  const startedAt = performance.now();
  const scheduleNext = (failed = false) => {
    if (session !== pointingCameraFrameSession || fields.pointingCameraWindow?.hidden) return;
    const targetIntervalMs = failed ? 500 : pointingCameraFrameIntervalMs;
    const delay = Math.max(0, targetIntervalMs - (performance.now() - startedAt));
    pointingCameraFrameTimers.set(camera, setTimeout(
      () => attachPointingLatestFrame(image, camera, session),
      delay,
    ));
  };
  image.onload = () => {
    if (session !== pointingCameraFrameSession) return;
    image.classList.add("is-ready");
    image.classList.remove("is-error");
    syncPointingCameraFeedLayout();
    scheduleNext(false);
  };
  image.onerror = () => {
    if (session !== pointingCameraFrameSession) return;
  ...
  image.src = `/api/camera-frame/latest/live/${camera}?preview=pointing&_=${Date.now()}`;
}

function pointingCameraZoomElement(camera) {
  return camera === "thermal"
    ? fields.pointingCameraThermalZoomPct
    : fields.pointingCameraVisibleZoomPct;
}

function updatePointingCameraZoom(camera, metadata) {
  const element = pointingCameraZoomElement(camera);
  if (!element) return;
  const percentage = cameraZoomPercentage(metadata);
  if (percentage === null) {
    element.textContent = "ZOOM --.-%";
    element.removeAttribute("title");
    return;
  }
  element.textContent = `ZOOM ${percentage.toFixed(1)}%`;
  const details = [];
  if (Number.isFinite(Number(metadata?.zoom_x))) details.push(`${Number(metadata.zoom_x).toFixed(2)}x`);
  if (Number.isFinite(Number(metadata?.focal_length_mm))) details.push(`${Number(metadata.focal_length_mm).toFixed(1)} mm`);
  element.title = details.length ? details.join(" · ") : `Zoom ${percentage.toFixed(1)}%`;
}

function subscribePointingCameraMetadata(camera) {
  pointingCameraMetadataStreams.get(camera)?.close();
  const stream = new EventSource(`/api/camera-stream/metadata-stream/${cameraSourceMode}/${camera}`);
  pointingCameraMetadataStreams.set(camera, stream);
  stream.onmessage = (event) => {
    if (pointingCameraMetadataStreams.get(camera) !== stream || fields.pointingCameraWindow?.hidden) return;
    let result;
    try {
    ...
    cameraStreamMetadata[camera] = metadata;
    updatePointingCameraZoom(camera, metadata);
    renderPointingModelZoomStatus();
  };
}

function pointingCameraStreamsNeedReconnect() {
  return [fields.pointingCameraThermalImage, fields.pointingCameraVisibleImage]
    .some((image) => image && (!image.getAttribute("src") || image.classList.contains("is-error")));
}

let pointingCameraResizeObserver = null;

function pointingCameraConfiguredRatio(feedSelector, fallbackWidth, fallbackHeight) {
  const resolution = fields.pointingCameraWindow
    ?.querySelector(`${feedSelector} > header em`)
    ?.textContent
    ?.match(/(\d+)\D+(\d+)/);
  const width = Number(resolution?.[1]) || fallbackWidth;
  const height = Number(resolution?.[2]) || fallbackHeight;
  return width / height;
}

function syncPointingCameraFeedLayout() {
  const windowElement = fields.pointingCameraWindow;
  const body = windowElement?.querySelector(".pointing-camera-body");
  if (!body || windowElement.hidden || !windowElement.classList.contains("is-docked")) return;

  // The MJPEG preview may be resized or cropped by the server. Use the camera's
  // configured source resolution so that a transport frame can never distort
  // the presentation aspect ratio.
  const thermalRatio = pointingCameraConfiguredRatio(".pointing-camera-feed.is-thermal", 640, 512);
  const visibleRatio = pointingCameraConfiguredRatio(".pointing-camera-feed.is-visible", 1920, 1080);
  const feeds = [...body.querySelectorAll(".pointing-camera-feed")];
  const viewports = feeds.map((feed) => feed.querySelector(".pointing-camera-viewport"));
  if (viewports[0]) viewports[0].style.aspectRatio = String(thermalRatio);
  if (viewports[1]) viewports[1].style.aspectRatio = String(visibleRatio);

  const style = getComputedStyle(body);
  const numberStyle = (name) => Number.parseFloat(style.getPropertyValue(name)) || 0;
  const horizontalInsets = numberStyle("padding-left") + numberStyle("padding-right");
  const verticalInsets = numberStyle("padding-top") + numberStyle("padding-bottom");
  const rowGap = numberStyle("row-gap");
  const feedHeaderHeight = feeds.reduce(
    (total, feed) => total + (feed.querySelector(":scope > header")?.offsetHeight || 0),
    0,
  );
  const directionHeight = body.querySelector(".pointing-camera-direction")?.offsetHeight || 0;
  const fixedHeight = verticalInsets + feedHeaderHeight + directionHeight
    + rowGap * Math.max(0, body.children.length - 1);
  const availableViewportHeight = Math.max(0, body.clientHeight - fixedHeight);
  const feedWidth = availableViewportHeight / (1 / thermalRatio + 1 / visibleRatio);
  const dockBorderWidth = Math.max(0, windowElement.offsetWidth - body.clientWidth);
  const dockWidth = Math.max(350, Math.min(460, Math.floor(feedWidth + horizontalInsets + dockBorderWidth)));
  document.body.style.setProperty("--pointing-camera-dock-width", `${dockWidth}px`);
}

function observePointingCameraLayout() {
  if (pointingCameraResizeObserver || typeof ResizeObserver !== "function") return;
  const body = fields.pointingCameraWindow?.querySelector(".pointing-camera-body");
  if (!body) return;
  pointingCameraResizeObserver = new ResizeObserver(() => syncPointingCameraFeedLayout());
  pointingCameraResizeObserver.observe(body);
}

function startPointingCameraStreams() {
  if (!fields.pointingCameraWindow || fields.pointingCameraWindow.hidden || document.hidden) return;
  releasePointingCameraStreams();
  const frameSession = pointingCameraFrameSession;
  const streamSession = `${Date.now()}-${Math.random().toString(16).slice(2)}`;
  if (fields.pointingCameraSource) {
    fields.pointingCameraSource.textContent = cameraSourceMode === "simulation" ? "SIMULATION" : "LIVE CAMERA";
  }
  [
    [fields.pointingCameraThermalImage, "thermal"],
    [fields.pointingCameraVisibleImage, "visible"],
  ].forEach(([image, camera]) => {
    if (!image) return;
    if (cameraSourceMode === "live") {
      attachPointingLatestFrame(image, camera, frameSession);
    } else {
      image.onload = () => {
        image.classList.add("is-ready");
        image.classList.remove("is-error");
      ...
      image.src = `/api/camera-mjpeg/${cameraSourceMode}/${camera}?preview=pointing&pointing=${streamSession}`;
    }
    subscribePointingCameraMetadata(camera);
  });
}

async function openPointingCameraWindow() {
  if (!fields.pointingCameraWindow) return;
  fields.pointingCameraWindow.hidden = false;
  document.body.classList.toggle(
    "is-pointing-camera-docked",
    fields.pointingCameraWindow.classList.contains("is-docked"),
  );
  fields.pointingCameraOpen?.classList.add("is-open");
  fields.pointingCameraOpen?.setAttribute("aria-expanded", "true");
  if (!fields.pointingCameraWindow.classList.contains("is-docked")) {
    restoreFloatingWindowPosition(fields.pointingCameraWindow, {
      left: Math.max(220, window.innerWidth - fields.pointingCameraWindow.offsetWidth - 36),
      top: 92,
    });
  }
  observePointingCameraLayout();
  requestAnimationFrame(() => {
    syncPointingCameraFeedLayout();
    window.pointingTerrain3D?.resize?.();
  });
  await selectAvailableCameraSource();
  startPointingCameraStreams();
  addLog("message", "Pointing Model camera view opened");
}

function closePointingCameraWindow() {
  if (!fields.pointingCameraWindow) return;
  if (activeDirectionJog && fields.pointingCameraWindow.contains(activeDirectionJog.button)) {
    stopActiveDirectionJog();
  }
  releasePointingCameraStreams();
  fields.pointingCameraWindow.hidden = true;
  document.body.classList.remove("is-pointing-camera-docked");
  document.body.style.removeProperty("--pointing-camera-dock-width");
  fields.pointingCameraOpen?.classList.remove("is-open");
  fields.pointingCameraOpen?.setAttribute("aria-expanded", "false");
  requestAnimationFrame(() => window.pointingTerrain3D?.resize?.());
}

function cameraViewShouldStream() {
  return Boolean(
    navigator.onLine
    && !dashboardConnectionSuspended
    && !document.hidden
    && fields.cameraLiveWindow
    && !fields.cameraLiveWindow.hidden
  );
}
...
const cameraMetadataGroups = [
  ["FRAME IDENTITY", ["captured_at", "camera_id", "width", "height", "resolution", "producer_sequence", "capture_sequence"]],
  ["OPTICS & FIELD OF VIEW", ["focal_length_mm", "zoom_ratio", "zoom_pct", "zoom_x", "fov_h_deg", "fov_v_deg", "fov_d_deg", "is_zooming"]],
  ["MOUNT & MISSION", ["azimuth_deg", "altitude_deg", "mission_state", "scan_mode", "is_paused", "current_waypoint", "total_waypoints", "current_layer", "total_layers"]],
  ["DETECTION", ["fire_detected"]],
];
let cameraMetadataRequest = 0;
let cameraMetadataEventSource = null;

function closeCameraMetadata() {
  cameraMetadataRequest += 1;
  cameraMetadataEventSource?.close();
  cameraMetadataEventSource = null;
  if (fields.cameraMetadataPanel) fields.cameraMetadataPanel.hidden = true;
}

function positionCameraMetadata(clientX, clientY) {
  const panel = fields.cameraMetadataPanel;
...
function formatCameraMetadataValue(key, value) {
  if (value === null || value === undefined || value === "") return "—";
  if (typeof value === "boolean") return value ? "TRUE" : "FALSE";
  if (["width", "height"].includes(key) && Number.isFinite(Number(value))) return `${value} px`;
  if (key === "focal_length_mm" && Number.isFinite(Number(value))) return `${Number(value).toFixed(1)} mm`;
  if (key === "zoom_ratio" && Number.isFinite(Number(value))) return Number(value).toFixed(3);
  if (key === "zoom_pct" && Number.isFinite(Number(value))) return `${Number(value).toFixed(1)} %`;
  if (key === "zoom_x" && Number.isFinite(Number(value))) return `${Number(value).toFixed(2)} x`;
  if (key.endsWith("_deg") && Number.isFinite(Number(value))) return `${Number(value).toFixed(2)}°`;
  if (typeof value === "object") return JSON.stringify(value);
  return String(value);
}

function appendCameraMetadataGroup(fragment, title, keys, metadata) {
  const availableKeys = keys.filter((key) => Object.prototype.hasOwnProperty.call(metadata, key));
  if (!availableKeys.length) return;
  const heading = document.createElement("div");
  heading.className = "camera-metadata-group-title";
  heading.textContent = title;
  fragment.appendChild(heading);
  availableKeys.forEach((key) => {
    const value = metadata[key];
    const row = document.createElement("div");
    row.className = "camera-metadata-row";
    const keyElement = document.createElement("span");
    keyElement.className = "camera-metadata-key";
    keyElement.textContent = key;
    keyElement.title = key;
    const valueElement = document.createElement("span");
    valueElement.className = "camera-metadata-value";
    valueElement.dataset.metadataKey = key;
    if (typeof value === "boolean") valueElement.classList.add(value ? "is-true" : "is-false");
    valueElement.textContent = formatCameraMetadataValue(key, value);
    row.append(keyElement, valueElement);
    fragment.appendChild(row);
  });
}

function renderCameraMetadata(metadata) {
  const body = fields.cameraMetadataBody;
  if (!body) return;
  const signature = Object.keys(metadata).sort().join("\u0000");
  if (body.dataset.metadataSignature === signature) {
    body.querySelectorAll("[data-metadata-key]").forEach((valueElement) => {
      const key = valueElement.dataset.metadataKey;
      const value = metadata[key];
      valueElement.classList.toggle("is-true", value === true);
      valueElement.classList.toggle("is-false", value === false);
      valueElement.textContent = formatCameraMetadataValue(key, value);
    });
    return;
  }
  const fragment = document.createDocumentFragment();
  const grouped = new Set(cameraMetadataGroups.flatMap(([, keys]) => keys));
  cameraMetadataGroups.forEach(([title, keys]) => appendCameraMetadataGroup(fragment, title, keys, metadata));
  const additionalKeys = Object.keys(metadata).filter((key) => !grouped.has(key)).sort();
  appendCameraMetadataGroup(fragment, "ADDITIONAL METADATA", additionalKeys, metadata);
  if (!fragment.childNodes.length) {
    const message = document.createElement("p");
    message.className = "camera-metadata-message";
    message.textContent = "This frame did not include metadata fields.";
    fragment.appendChild(message);
  }
  body.replaceChildren(fragment);
  body.dataset.metadataSignature = signature;
}

function openCameraMetadata(camera, clientX, clientY) {
  const panel = fields.cameraMetadataPanel;
  if (!panel || !fields.cameraMetadataBody) return;
  cameraMetadataEventSource?.close();
  cameraMetadataEventSource = null;
  const requestId = ++cameraMetadataRequest;
  const label = camera === "thermal" ? "THERMAL" : "VISIBLE";
  panel.classList.toggle("is-thermal", camera === "thermal");
  if (fields.cameraMetadataTitle) fields.cameraMetadataTitle.textContent = `${label} FRAME METADATA`;
  if (fields.cameraMetadataSource) fields.cameraMetadataSource.textContent = `${cameraSourceMode.toUpperCase()} · requesting latest frame`;
  const loading = document.createElement("p");
  loading.className = "camera-metadata-message";
  loading.textContent = "Subscribing to live frame metadata…";
  delete fields.cameraMetadataBody.dataset.metadataSignature;
  fields.cameraMetadataBody.replaceChildren(loading);
  panel.hidden = false;
  requestAnimationFrame(() => positionCameraMetadata(clientX, clientY));

  const stream = new EventSource(`/api/camera-stream/metadata-stream/${cameraSourceMode}/${camera}`);
  cameraMetadataEventSource = stream;
  let receivedFrame = false;
  stream.onmessage = (event) => {
    if (requestId !== cameraMetadataRequest || panel.hidden || stream !== cameraMetadataEventSource) return;
    let result;
    try {
      result = JSON.parse(event.data);
    } catch (_error) {
    ...
    cameraStreamMetadata[camera] = metadata;
    renderPointingModelZoomStatus();
    if (fields.cameraMetadataSource) {
      const age = Number.isFinite(Number(result.age_ms)) ? ` · age ${Number(result.age_ms).toFixed(0)} ms` : "";
      fields.cameraMetadataSource.textContent = `${String(result.source).toUpperCase()} · LIVE · frame #${result.sequence}${age}`;
    }
    renderCameraMetadata(metadata);
    requestAnimationFrame(() => positionCameraMetadata(clientX, clientY));
  };
  stream.onerror = () => {
    if (requestId !== cameraMetadataRequest || panel.hidden || stream !== cameraMetadataEventSource) return;
    if (receivedFrame) {
      if (fields.cameraMetadataSource) fields.cameraMetadataSource.textContent = `${cameraSourceMode.toUpperCase()} · reconnecting metadata stream…`;
      return;
    }
    const message = document.createElement("p");
    message.className = "camera-metadata-message is-error";
    message.textContent = "Waiting for live frame metadata…";
    delete fields.cameraMetadataBody.dataset.metadataSignature;
    fields.cameraMetadataBody.replaceChildren(message);
    if (fields.cameraMetadataSource) fields.cameraMetadataSource.textContent = `${cameraSourceMode.toUpperCase()} · metadata stream unavailable`;
  };
}

function cameraStreamLoader(camera) {
  return fields.cameraLiveWindow?.querySelector(`[data-camera-loader="${camera}"]`) || null;
}

function setCameraLoaderState(camera, state) {
  const loader = cameraStreamLoader(camera);
  if (!loader) return;
  const cameraName = camera === "thermal" ? "Thermal" : "Visible";
  const title = loader.querySelector("strong");
  const detail = loader.querySelector("small");
  loader.classList.toggle("is-reconnecting", state === "reconnecting");
  loader.hidden = state === "ready" || state === "paused";
  if (title) title.textContent = `${state === "reconnecting" ? "Reconnecting" : "Connecting"} ${cameraName} Camera`;
  if (detail) detail.textContent = state === "reconnecting"
    ? "JPEG stream interrupted — retrying…"
    : "Establishing JPEG stream…";
}

function clearCameraStreamRetry(camera) {
  clearTimeout(cameraStreamRetryTimers.get(camera));
  cameraStreamRetryTimers.delete(camera);
}

function releaseCameraStreams({ statusMessage = "Camera streams paused" } = {}) {
  cameraStreamsWanted = false;
  cameraStreamSession += 1;
  cameraStreamReady.clear();
  ["thermal", "visible"].forEach((camera) => {
    clearCameraStreamRetry(camera);
    setCameraLoaderState(camera, "paused");
  });
  [fields.thermalLiveImage, fields.visibleLiveImage, fields.thermalRealImage, fields.visibleRealImage]
    .forEach((image) => {
      ...
      delete image.dataset.hasFrame;
    });
  if (statusMessage) setCameraConnectionStatus(statusMessage);
}

function updateCameraStreamReadyStatus(camera) {
  const readyCount = cameraStreamReady.size;
  setCameraConnectionStatus(
    readyCount >= 2
      ? `AIV ${cameraSourceMode} JPEG streams connected`
      : `AIV ${camera} JPEG stream connected (${readyCount}/2)`,
    true,
  );
}

function attachCameraImageStream(image, camera, session, attempt = 0) {
  if (!image || session !== cameraStreamSession || !cameraStreamsWanted || !cameraViewShouldStream()) return;
  clearCameraStreamRetry(camera);
  image.onload = () => {
    if (session !== cameraStreamSession || !cameraStreamsWanted) return;
    image.dataset.hasFrame = "true";
    image.classList.remove("is-stream-disconnected");
    cameraStreamReady.add(camera);
    setCameraLoaderState(camera, "ready");
    updateCameraStreamReadyStatus(camera);
  };
  image.onerror = () => {
    if (session !== cameraStreamSession || !cameraStreamsWanted || !cameraViewShouldStream()) return;
    cameraStreamReady.delete(camera);
    const hasLastFrame = image.dataset.hasFrame === "true";
    image.classList.toggle("is-stream-disconnected", !hasLastFrame);
    setCameraLoaderState(camera, attempt > 0 || hasLastFrame ? "reconnecting" : "connecting");
    setCameraConnectionStatus(`AIV ${camera} JPEG stream interrupted — retrying…`);
    const retryDelayMs = Math.min(5000, 750 * (2 ** Math.min(attempt, 3)));
    cameraStreamRetryTimers.set(camera, setTimeout(() => {
      attachCameraImageStream(image, camera, session, attempt + 1);
    }, retryDelayMs));
  };
  image.src = `/api/camera-mjpeg/${cameraSourceMode}/${camera}?session=${session}-${attempt}-${Date.now()}`;
}

function startCameraStreams() {
  releaseCameraStreams({ statusMessage: null });
  if (!cameraViewShouldStream()) return;
  cameraStreamsWanted = true;
  const session = cameraStreamSession;
  setCameraConnectionStatus(`Connecting to AIV ${cameraSourceMode} JPEG streams…`);
  cameraStreamTargets().forEach(([image, camera]) => {
    if (!image) return;
    image.classList.add("is-stream-disconnected");
    setCameraLoaderState(camera, "connecting");
    attachCameraImageStream(image, camera, session);
  });
}
...
function connectCameraStream() {
  if (!cameraStreamsWanted) startCameraStreams();
}
...
function renderCameraSourceMode() {
  ...
  releaseCameraStreams({ statusMessage: null });
  if (!fields.cameraLiveWindow?.hidden) startCameraStreams();
  if (!fields.pointingCameraWindow?.hidden) startPointingCameraStreams();
}
...
async function selectAvailableCameraSource() {
  ...
  try {
    ...
    const sources = poolStatus.sources || {};
    const pointingMaxFps = Number(poolStatus.profiles?.pointing?.max_fps);
    if (Number.isFinite(pointingMaxFps) && pointingMaxFps > 0) {
      pointingCameraFrameIntervalMs = Math.max(50, Math.round(1000 / pointingMaxFps));
    }
...
function cameraCalibrationNumber(input, fallback, min, max) {
...
function thermalCalibrationResolution() {
  const source = cameraCalibrationSource("thermal");
  const metadata = appSettings.camera_metadata?.thermal_camera || {};
  const width = Math.round(Number(metadata.native_width || source?.naturalWidth || 640));
  const height = Math.round(Number(metadata.native_height || source?.naturalHeight || 512));
  return {
    width: Math.max(1, Number.isFinite(width) ? width : 640),
    height: Math.max(1, Number.isFinite(height) ? height : 512),
  };
}

function visibleCalibrationResolution() {
  const source = cameraCalibrationSource("visible");
  const metadata = appSettings.camera_metadata?.visible_camera || {};
  const width = Math.round(Number(metadata.native_width || source?.naturalWidth || 1920));
  const height = Math.round(Number(metadata.native_height || source?.naturalHeight || 1080));
  return {
...
function cameraCalibrationCropForAspect(value, changedDimension = "width") {
  const original = visibleCalibrationResolution();
  const numericValue = Math.round(Number(value));
  if (!Number.isFinite(numericValue) || numericValue < 1) return null;
  if (changedDimension === "height") {
    const height = Math.min(original.height, numericValue);
    return {
      width: Math.max(1, Math.min(original.width, Math.round(height * original.width / original.height))),
      height,
    };
  }
  const width = Math.min(original.width, numericValue);
  return {
    width,
    height: Math.max(1, Math.min(original.height, Math.round(width * original.height / original.width))),
  };
}

function synchronizeCameraCalibrationCrop(changedInput) {
  const changedDimension = changedInput === fields.cameraCalibrationCropHeight ? "height" : "width";
  const crop = cameraCalibrationCropForAspect(changedInput?.value, changedDimension);
  if (!crop) return false;
  if (fields.cameraCalibrationCropWidth) fields.cameraCalibrationCropWidth.value = String(crop.width);
  if (fields.cameraCalibrationCropHeight) fields.cameraCalibrationCropHeight.value = String(crop.height);
  return true;
}

function configureCameraCalibrationPreviewGeometry() {
  const thermal = thermalCalibrationResolution();
  const visible = visibleCalibrationResolution();
  const sharedHeight = thermal.height;
  const normalizedVisibleWidth = Math.max(1, Math.round(sharedHeight * visible.width / visible.height));
  if (fields.cameraCalibrationThermalCanvas) {
    if (fields.cameraCalibrationThermalCanvas.width !== thermal.width) fields.cameraCalibrationThermalCanvas.width = thermal.width;
    if (fields.cameraCalibrationThermalCanvas.height !== sharedHeight) fields.cameraCalibrationThermalCanvas.height = sharedHeight;
  }
  if (fields.cameraCalibrationVisibleCanvas) {
    if (fields.cameraCalibrationVisibleCanvas.width !== normalizedVisibleWidth) fields.cameraCalibrationVisibleCanvas.width = normalizedVisibleWidth;
    if (fields.cameraCalibrationVisibleCanvas.height !== sharedHeight) fields.cameraCalibrationVisibleCanvas.height = sharedHeight;
  }
  if (fields.cameraCalibrationPreviews) {
    fields.cameraCalibrationPreviews.style.gridTemplateColumns = `minmax(0, ${thermal.width}fr) minmax(0, ${normalizedVisibleWidth}fr)`;
  }
  if (fields.cameraCalibrationVisibleResolution) {
    fields.cameraCalibrationVisibleResolution.textContent = `VISIBLE ${visible.width} × ${visible.height} · PREVIEW ${normalizedVisibleWidth} × ${sharedHeight}`;
  }
  if (fields.cameraCalibrationCropWidth) fields.cameraCalibrationCropWidth.max = String(visible.width);
  if (fields.cameraCalibrationCropHeight) fields.cameraCalibrationCropHeight.max = String(visible.height);
  return { thermal, visible, sharedHeight, normalizedVisibleWidth };
}

function cameraCalibrationValues() {
  const thermalResolution = thermalCalibrationResolution();
  return {
    visible_offset_x_px: cameraCalibrationNumber(fields.cameraCalibrationOffsetX, 0, -32768, 32768),
    visible_offset_y_px: cameraCalibrationNumber(fields.cameraCalibrationOffsetY, 0, -32768, 32768),
    step_px: cameraCalibrationNumber(fields.cameraCalibrationStep, 10, 1, 1000),
    visible_crop_width_px: cameraCalibrationNumber(fields.cameraCalibrationCropWidth, thermalResolution.width, 1, 32768),
    visible_crop_height_px: cameraCalibrationNumber(fields.cameraCalibrationCropHeight, thermalResolution.height, 1, 32768),
  };
}

function setCameraCalibrationValues(calibration = {}) {
  const thermalResolution = thermalCalibrationResolution();
  if (fields.cameraCalibrationOffsetX) fields.cameraCalibrationOffsetX.value = String(Math.round(Number(calibration.visible_offset_x_px) || 0));
  if (fields.cameraCalibrationOffsetY) fields.cameraCalibrationOffsetY.value = String(Math.round(Number(calibration.visible_offset_y_px) || 0));
  if (fields.cameraCalibrationStep) fields.cameraCalibrationStep.value = String(Math.max(1, Math.round(Number(calibration.step_px) || 10)));
  if (fields.cameraCalibrationCropWidth) {
    fields.cameraCalibrationCropWidth.value = String(Math.max(1, Math.round(Number(calibration.visible_crop_width_px) || thermalResolution.width)));
  }
  if (fields.cameraCalibrationCropHeight) {
    fields.cameraCalibrationCropHeight.value = String(Math.max(1, Math.round(Number(calibration.visible_crop_height_px) || thermalResolution.height)));
  }
  // Normalize legacy saved values to the original Visible aspect ratio. The
  // saved width is authoritative when the calibration window first opens.
  synchronizeCameraCalibrationCrop(fields.cameraCalibrationCropWidth);
  updateCameraCalibrationPositionLabel();
}

function updateCameraCalibrationPositionLabel() {
  if (!fields.cameraCalibrationVisiblePosition) return;
  const values = cameraCalibrationValues();
  const signed = (value) => `${value >= 0 ? "+" : ""}${value}`;
  fields.cameraCalibrationVisiblePosition.textContent = `X ${signed(values.visible_offset_x_px)} px · Y ${signed(values.visible_offset_y_px)} px · ${values.visible_crop_width_px} × ${values.visible_crop_height_px}`;
}

function cameraCalibrationSource(camera) {
  return cameraStreamTargets().find(([, name]) => name === camera)?.[0] || null;
}

function drawCalibrationCrosshair(context, x, y, color) {
  const arm = 34;
  const gap = 7;
  context.save();
...
function drawCalibrationCropBox(context, centerX, centerY, cropWidth, cropHeight) {
  const left = centerX - cropWidth / 2;
  const top = centerY - cropHeight / 2;
  context.save();
  context.fillStyle = "rgba(255, 176, 82, .055)";
  context.strokeStyle = "rgba(255, 176, 82, .92)";
  context.lineWidth = 2;
  context.setLineDash([10, 6]);
  context.shadowColor = "rgba(255, 176, 82, .72)";
  context.shadowBlur = 5;
  context.fillRect(left, top, cropWidth, cropHeight);
  context.strokeRect(left, top, cropWidth, cropHeight);
  context.setLineDash([]);

  const corner = Math.min(20, Math.max(8, Math.min(cropWidth, cropHeight) * .12));
  context.lineWidth = 3;
  context.beginPath();
  context.moveTo(left, top + corner); context.lineTo(left, top); context.lineTo(left + corner, top);
  context.moveTo(left + cropWidth - corner, top); context.lineTo(left + cropWidth, top); context.lineTo(left + cropWidth, top + corner);
  context.moveTo(left + cropWidth, top + cropHeight - corner); context.lineTo(left + cropWidth, top + cropHeight); context.lineTo(left + cropWidth - corner, top + cropHeight);
  context.moveTo(left + corner, top + cropHeight); context.lineTo(left, top + cropHeight); context.lineTo(left, top + cropHeight - corner);
  context.stroke();
  context.restore();
}

function drawCameraCalibrationPreview(canvas, source, camera, values) {
  if (!canvas) return;
  ...
  if (sourceWidth <= 0 || sourceHeight <= 0) {
    context.fillStyle = "#71828a";
    context.font = "700 18px sans-serif";
    context.textAlign = "center";
    context.fillText(`WAITING FOR ${camera.toUpperCase()} FRAME`, width / 2, height / 2);
    return;
  }
  ...
  context.fillRect(0, 0, width, height);
  const configuredResolution = camera === "visible"
    ? visibleCalibrationResolution()
    : thermalCalibrationResolution();
  const coordinateScaleX = width / configuredResolution.width;
  const coordinateScaleY = height / configuredResolution.height;
  const offsetX = camera === "visible" ? values.visible_offset_x_px * coordinateScaleX : 0;
  const offsetY = camera === "visible" ? values.visible_offset_y_px * coordinateScaleY : 0;
  const centerX = width / 2 + offsetX;
  const centerY = height / 2 + offsetY;
  if (camera === "visible") {
    drawCalibrationCropBox(
      context,
      centerX,
      centerY,
      values.visible_crop_width_px * coordinateScaleX,
      values.visible_crop_height_px * coordinateScaleY,
    );
  }
  drawCalibrationCrosshair(
    context,
    centerX,
    centerY,
    camera === "thermal" ? "#62e38d" : "#ffb052",
  );
}

function refreshCameraCalibration(renderAt = performance.now()) {
  if (!fields.cameraCalibrationWindow || fields.cameraCalibrationWindow.hidden) {
    cameraCalibrationAnimationFrame = null;
    return;
  }
  if (renderAt - cameraCalibrationLastRenderAt >= 100) {
    cameraCalibrationLastRenderAt = renderAt;
    configureCameraCalibrationPreviewGeometry();
    const values = cameraCalibrationValues();
    drawCameraCalibrationPreview(
      fields.cameraCalibrationThermalCanvas,
      cameraCalibrationSource("thermal"),
      "thermal",
      values,
    );
    drawCameraCalibrationPreview(
      fields.cameraCalibrationVisibleCanvas,
      cameraCalibrationSource("visible"),
      "visible",
      values,
    );
    updateCameraCalibrationPositionLabel();
  }
  cameraCalibrationAnimationFrame = requestAnimationFrame(refreshCameraCalibration);
}

function setCameraCalibrationMessage(message, state = "") {
  if (!fields.cameraCalibrationMessage) return;
  fields.cameraCalibrationMessage.classList.remove("is-ok", "is-error");
  if (state) fields.cameraCalibrationMessage.classList.add(state);
  fields.cameraCalibrationMessage.textContent = message;
}

async function openCameraCalibrationWindow() {
  if (!fields.cameraCalibrationWindow) return;
  if (fields.cameraLiveWindow?.hidden) await openCameraLiveWindow();
  await loadAppSettings();
  configureCameraCalibrationPreviewGeometry();
  setCameraCalibrationValues(appSettings.camera_calibration);
  setCameraCalibrationMessage("Adjust the Visible center, then save the alignment.");
  fields.cameraCalibrationWindow.hidden = false;
  fields.cameraCalibrationOpen?.setAttribute("aria-expanded", "true");
  restoreFloatingWindowPosition(fields.cameraCalibrationWindow, {
    left: Math.max(12, (window.innerWidth - Math.min(1456, window.innerWidth - 32)) / 2),
    top: Math.max(12, (window.innerHeight - Math.min(840, window.innerHeight - 32)) / 2),
  });
  cameraCalibrationLastRenderAt = 0;
  cancelAnimationFrame(cameraCalibrationAnimationFrame);
  refreshCameraCalibration();
  addLog("message", "Cameras calibration opened");
}

function closeCameraCalibrationWindow() {
  if (!fields.cameraCalibrationWindow) return;
  fields.cameraCalibrationWindow.hidden = true;
  fields.cameraCalibrationOpen?.setAttribute("aria-expanded", "false");
  cancelAnimationFrame(cameraCalibrationAnimationFrame);
  cameraCalibrationAnimationFrame = null;
}

function nudgeCameraCalibration(direction) {
  const values = cameraCalibrationValues();
  if (direction === "up") values.visible_offset_y_px -= values.step_px;
  if (direction === "down") values.visible_offset_y_px += values.step_px;
  if (direction === "left") values.visible_offset_x_px -= values.step_px;
  if (direction === "right") values.visible_offset_x_px += values.step_px;
  setCameraCalibrationValues(values);
  setCameraCalibrationMessage("Visible center changed. Save to keep this alignment.");
}

function resetCameraCalibrationCrop() {
  const values = cameraCalibrationValues();
  const visibleResolution = visibleCalibrationResolution();
  setCameraCalibrationValues({
    ...values,
    visible_crop_width_px: visibleResolution.width,
    visible_crop_height_px: visibleResolution.height,
  });
  setCameraCalibrationMessage(
    `Visible crop reset to Original resolution ${visibleResolution.width} × ${visibleResolution.height} px. Save to keep this size.`,
  );
}
...
async function saveCameraCalibration() {
  const values = cameraCalibrationValues();
  setCameraCalibrationValues(values);
  if (fields.cameraCalibrationSave) fields.cameraCalibrationSave.disabled = true;
  setCameraCalibrationMessage("Saving cameras calibration…");
  try {
    const response = await fetch("/api/settings", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ camera_calibration: values, flash_drives: false }),
    });
    const data = await response.json();
    if (!response.ok || !data.ok) throw new Error(data.error || "Calibration settings update failed.");
    appSettings = data.settings;
    setCameraCalibrationValues(appSettings.camera_calibration);
    setCameraCalibrationMessage("Cameras calibration saved to system settings.", "is-ok");
    addLog("success", `Camera alignment saved: X ${values.visible_offset_x_px} px, Y ${values.visible_offset_y_px} px, crop ${values.visible_crop_width_px} × ${values.visible_crop_height_px} px, step ${values.step_px} px`);
  } catch (error) {
    setCameraCalibrationMessage(error.message, "is-error");
    addLog("error", `Cameras calibration save failed: ${error.message}`);
  } finally {
    if (fields.cameraCalibrationSave) fields.cameraCalibrationSave.disabled = false;
  }
}
...
fields.cameraMetadataClose?.addEventListener("click", closeCameraMetadata);
[
  [fields.cameraLiveWindow?.querySelector(".thermal-feed .camera-viewport"), "thermal"],
  [fields.cameraLiveWindow?.querySelector(".visible-feed .camera-viewport"), "visible"],
].forEach(([viewport, camera]) => {
  viewport?.addEventListener("contextmenu", (event) => {
    event.preventDefault();
    openCameraMetadata(camera, event.clientX, event.clientY);
  });
});
fields.cameraCalibrationOpen?.addEventListener("click", () => {
  if (fields.cameraCalibrationWindow?.hidden) openCameraCalibrationWindow();
  else closeCameraCalibrationWindow();
});
fields.cameraCalibrationClose?.addEventListener("click", closeCameraCalibrationWindow);
fields.cameraCalibrationReset?.addEventListener("click", resetCameraCalibrationCrop);
fields.cameraCalibrationSave?.addEventListener("click", saveCameraCalibration);
fields.cameraCalibrationWindow?.querySelectorAll("[data-calibration-nudge]").forEach((button) => {
  button.addEventListener("click", () => nudgeCameraCalibration(button.dataset.calibrationNudge));
});
fields.cameraCalibrationWindow?.querySelector("[data-calibration-center]")?.addEventListener("click", () => {
  const values = cameraCalibrationValues();
  setCameraCalibrationValues({ ...values, visible_offset_x_px: 0, visible_offset_y_px: 0 });
  setCameraCalibrationMessage("Visible center reset to the image center. Save to keep this alignment.");
});
[
  fields.cameraCalibrationOffsetX,
  fields.cameraCalibrationOffsetY,
  fields.cameraCalibrationStep,
  fields.cameraCalibrationCropWidth,
  fields.cameraCalibrationCropHeight,
].forEach((input) => {
  input?.addEventListener("input", () => {
    if (input === fields.cameraCalibrationCropWidth || input === fields.cameraCalibrationCropHeight) {
      synchronizeCameraCalibrationCrop(input);
    }
    updateCameraCalibrationPositionLabel();
    setCameraCalibrationMessage("Calibration values changed. Save to keep this alignment.");
  });
});
...
fields.directionMode?.addEventListener("change", updateDirectionControlMode);
fields.directionSpeed?.addEventListener("input", () => {
  directionModeValues[directionControlMode()] = fields.directionSpeed.value;
});
fields.directionPanel?.querySelectorAll("[data-direction-az]").forEach((button) => {
  button.addEventListener("pointerdown", (event) => {
    if (event.button !== 0) return;
    event.preventDefault();
    if (directionControlMode() === "offset") sendDirectionOffsetCommand(button);
    else startDirectionJog(button, event);
  });
  button.addEventListener("click", (event) => event.preventDefault());
});
fields.directionPanel?.querySelector("[data-direction-stop]")?.addEventListener("click", () => {
  if (activeDirectionJog) stopActiveDirectionJog();
  else sendDirectionJogCommand(0, 0, directionControlMode() === "offset" ? "Mount stopped" : "Direction jog stopped");
});
updateDirectionControlMode();
fields.pointingCameraOpen?.addEventListener("click", () => {
  if (fields.pointingCameraWindow?.hidden) openPointingCameraWindow();
  else closePointingCameraWindow();
});
fields.pointingModelEditorOpen?.addEventListener("click", openPointingModelEditor);
fields.pointingModelEditorClose?.addEventListener("click", closePointingModelEditor);
fields.pointingCameraClose?.addEventListener("click", closePointingCameraWindow);
fields.pointingCameraPopout?.addEventListener("click", () => {
  const monitorWindow = window.open("/camera-monitor", "camera-monitor");
  if (!monitorWindow) {
    addLog("warning", "Camera Monitor tab was blocked by the browser.");
    return;
  }
  monitorWindow.opener = null;
  closePointingCameraWindow();
  addLog("message", "Camera View moved to the second-display tab");
});
fields.pointingCameraDirectionMode?.addEventListener("change", updatePointingCameraDirectionMode);
fields.pointingCameraDirectionValue?.addEventListener("input", () => {
  pointingCameraDirectionValues[pointingCameraDirectionMode()] = fields.pointingCameraDirectionValue.value;
});
fields.pointingCameraDirectionPad?.querySelectorAll("[data-camera-direction-az]").forEach((button) => {
  button.addEventListener("pointerdown", (event) => {
    if (event.button !== 0) return;
    event.preventDefault();
    const controls = {
      input: fields.pointingCameraDirectionValue,
      setMessage: setPointingCameraDirectionMessage,
    };
    if (pointingCameraDirectionMode() === "offset") sendDirectionOffsetCommand(button, controls);
    else startDirectionJog(button, event, controls);
  });
  button.addEventListener("click", (event) => event.preventDefault());
});
fields.pointingCameraDirectionPad?.querySelector("[data-camera-direction-stop]")?.addEventListener("click", () => {
  if (activeDirectionJog) stopActiveDirectionJog();
  else {
    sendDirectionJogCommand(
      0,
      0,
      pointingCameraDirectionMode() === "offset" ? "Mount stopped" : "Direction jog stopped",
      true,
      setPointingCameraDirectionMessage,
    );
  }
});
updatePointingCameraDirectionMode();
...
window.addEventListener("pointing-map-goto", () => {
  if (activeViewMode !== "pointing" || fields.pointingTargetGoto?.disabled) return;
  fields.pointingTargetGoto?.click();
});
window.addEventListener("pointing-map-add-model", openPointingModelComparison);
fields.pointingModelComparisonClose?.addEventListener("click", () => {
  if (fields.pointingModelComparisonPanel) fields.pointingModelComparisonPanel.hidden = true;
});
fields.pointingModelNext?.addEventListener("click", openPointingAlignment);
fields.pointingAlignmentClose?.addEventListener("click", () => closePointingAlignment());
fields.pointingAlignmentMinimize?.addEventListener("click", minimizePointingAlignment);
fields.pointingAlignmentRestore?.addEventListener("click", restorePointingAlignment);
fields.pointingAlignmentBack?.addEventListener("click", () => closePointingAlignment({ reopenComparison: true }));
fields.pointingAlignmentSave?.addEventListener("click", addPointingAlignmentCalculationPoint);
fields.pointingAlignmentDirectionMode?.addEventListener("change", updatePointingAlignmentDirectionMode);
fields.pointingAlignmentOffsetStep?.addEventListener("input", () => {
  pointingAlignmentDirectionValues[pointingAlignmentDirectionMode()] = fields.pointingAlignmentOffsetStep.value;
});
fields.pointingAlignmentDirectionPad?.querySelectorAll("[data-alignment-direction-az]").forEach((button) => {
  button.addEventListener("pointerdown", (event) => {
    if (event.button !== 0) return;
    event.preventDefault();
    const controls = {
      input: fields.pointingAlignmentOffsetStep,
      setMessage: setPointingAlignmentMessage,
      onAccepted: applyPointingAlignmentAcceptedOffset,
    };
    if (pointingAlignmentDirectionMode() === "offset") sendDirectionOffsetCommand(button, controls);
    else startDirectionJog(button, event, controls);
  });
  button.addEventListener("click", (event) => event.preventDefault());
});
fields.pointingAlignmentDirectionPad?.querySelector("[data-alignment-direction-stop]")?.addEventListener("click", () => {
  if (activeDirectionJog) stopActiveDirectionJog();
  else sendDirectionJogCommand(
    0,
    0,
    pointingAlignmentDirectionMode() === "offset" ? "Mount stopped" : "Direction jog stopped",
    true,
    setPointingAlignmentMessage,
  );
});
updatePointingAlignmentDirectionMode();
bindPointingModelZoomButton(fields.pointingModelZoomOut, "out");
bindPointingModelZoomButton(fields.pointingModelZoomIn, "in");
bindPointingModelZoomButton(fields.pointingAlignmentZoomOut, "out", fields.pointingAlignmentZoomStep);
bindPointingModelZoomButton(fields.pointingAlignmentZoomIn, "in", fields.pointingAlignmentZoomStep);
[fields.pointingModelLinkedZoomPct, fields.pointingAlignmentLinkedZoomPct].forEach((input) => {
  input?.addEventListener("focus", (event) => event.currentTarget.select());
  input?.addEventListener("blur", () => commitPointingModelLinkedZoom(input));
  input?.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      ...
      event.preventDefault();
      event.currentTarget.value = pointingModelLinkedZoomPct === null
        ? ""
        : pointingModelLinkedZoomPct.toFixed(1);
      event.currentTarget.blur();
    }
...
[fields.pointingModelZoomStep, fields.pointingAlignmentZoomStep].forEach((input) => {
  input?.addEventListener("change", () => {
    const step = pointingModelZoomStep(input);
    if (step !== null) {
      input.value = step.toFixed(1);
      setPointingModelZoomMessage("Click once for one step. Press and hold for continuous steps.");
    }
  });
});
...
fields.settingsApply?.addEventListener("click", applySystemSettings);
...
fields.settingsForm?.addEventListener("input", () => {
...
document.addEventListener("pointerdown", (event) => {
  if (
    fields.cameraMetadataPanel
    && !fields.cameraMetadataPanel.hidden
    && !fields.cameraMetadataPanel.contains(event.target)
    && !event.target.closest?.(".camera-viewport")
  ) closeCameraMetadata();
...
document.addEventListener("keydown", (event) => {
  if (event.key === "Escape") {
    if (fields.pointingModelEditor && !fields.pointingModelEditor.hidden) {
      event.preventDefault();
      event.stopPropagation();
      closePointingModelEditor();
      return;
    ...
    if (fields.pointingAlignmentPanel && !fields.pointingAlignmentPanel.hidden) {
      event.preventDefault();
      event.stopPropagation();
      closePointingAlignment();
      return;
    }
    if (fields.cameraMetadataPanel && !fields.cameraMetadataPanel.hidden) {
      event.preventDefault();
      event.stopPropagation();
      closeCameraMetadata();
      return;
...
document.addEventListener("pointerdown", () => {
  ...
  if (
    fields.pointingCameraWindow
    && !fields.pointingCameraWindow.hidden
    && pointingCameraStreamsNeedReconnect()
  ) {
    startPointingCameraStreams();
  }
  if (
    fields.pointingAlignmentPanel
    && !fields.pointingAlignmentPanel.hidden
    && pointingAlignmentStreamsNeedReconnect()
  ) {
    startPointingAlignmentStreams();
  }
}, { passive: true });
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) {
    ...
    if (fields.cameraLiveWindow && !fields.cameraLiveWindow.hidden) {
      releaseCameraStreams({ statusMessage: "Camera streams paused while page is hidden" });
    }
    if (fields.pointingCameraWindow && !fields.pointingCameraWindow.hidden) {
      releasePointingCameraStreams();
    }
    if (fields.pointingAlignmentPanel && !fields.pointingAlignmentPanel.hidden) {
      releasePointingAlignmentStreams();
  ...
  if (fields.cameraLiveWindow && !fields.cameraLiveWindow.hidden) startCameraStreams();
  if (fields.pointingCameraWindow && !fields.pointingCameraWindow.hidden) startPointingCameraStreams();
  if (fields.pointingAlignmentPanel && !fields.pointingAlignmentPanel.hidden) startPointingAlignmentStreams();
});
